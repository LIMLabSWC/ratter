...
switch action
  ...
  case 'update'    % --- CASE UPDATE -------------------
    if ~isempty(varargin)
        LocalLeftProb.value   = varargin{1};  
    end   
    if length(varargin)>1
        LocalHitHistory.value = varargin{2}; 
    end
    if length(varargin)>2
        LocalPrevSides.value  = varargin{3};
    end
    % Protect against somebody passing in SPHs, not actual values, by mistake:
    if isa(value(LocalLeftProb),   'SoloParamHandle')
        LocalLeftProb.value   = value(value(LocalLeftProb));
    end
    if isa(value(LocalHitHistory), 'SoloParamHandle')
        LocalHitHistory.value = value(value(LocalHitHistory)); 
    end
    if isa(value(LocalPrevSides),  'SoloParamHandle')
        LocalPrevSides.value  = value(value(LocalPrevSides));  
    end
  ...
  case 'update_hitfrac'     % ------- CASE UPDATE_HITFRAC -------------
    ...
    hit_history    = value(LocalHitHistory); 
    hit_history = colvec(hit_history);
    ...
    if ~isempty(hit_history) 
      ...
      if isempty(u)
          ...
          LtHitFrac.value = sum(hit_history(u) .* kernel(u))/sum(kernel(u));
      end
      ...
      if isempty(u)
          ...
          RtHitFrac.value = sum(hit_history(u) .* kernel(u))/sum(kernel(u));
      end
  ...
  case 'update_biashitfrac'     % ------- CASE UPDATE_BIASHITFRAC -------------
    ...
    if isempty(ul), BiasLtHitFrac.value = 1;
        ...
        BiasLtHitFrac.value = sum(hit_history(ul) .* kernel(ul))/sum(kernel(ul));
    end
        
    ur = find(prevs == 'r');
    if isempty(ur), BiasRtHitFrac.value = 1;
    else            
        BiasRtHitFrac.value = sum(hit_history(ur) .* kernel(ur))/sum(kernel(ur));
    end
    
    if isempty(ul) && ~isempty(ur)
        BiasLtHitFrac.value = value(BiasRtHitFrac); 
    end
    if isempty(ur) && ~isempty(ul)
        BiasRtHitFrac.value = value(BiasLtHitFrac); 
    end
