    ...
    for i = 1:numel(linegroups)
        if ~isempty(linegroups{i}) && ~isempty(linegroups{i}{1,2})
            ...
            if strcmp(linegroups{i}{1,2},'L') && ~isempty(soundserver)
                ToggleParam(obj,'LeftSound',        0,0,0,'position',[((i-1)*groupwidth)+1,  10, groupwidth-2,50],'OnString', 'Sound Two ON',        'OffString', 'Sound Two OFF');
                set_callback(LeftSound,{mfilename,'play_left_sound'});
                
            elseif strcmp(linegroups{i}{1,2},'R') && ~isempty(soundserver)
                ToggleParam(obj,'RightSound',       0,0,0,'position',[((i-1)*groupwidth)+1,  10, groupwidth-2,50],'OnString', 'Sound Three ON',       'OffString', 'Sound Three OFF');          
                set_callback(RightSound,{mfilename,'play_right_sound'});
                
            elseif strcmp(linegroups{i}{1,2},'C') && ~isempty(soundserver)
                ToggleParam(obj,'CenterSound',      0,0,0,'position',[((i-1)*groupwidth)+1,  10, groupwidth-2,50],'OnString', 'Sound One ON',       'OffString', 'Sound One OFF');          
