  ...
  case 'update',
    if n_done_trials > 0,
        if isfield(parsed_events.states, 'temporary_punishment') && ...
          ...
          if rows(parsed_events.states.hit_state) > 0, hit_history.value = [hit_history(:) ; 1]; %#ok<NODEF>
          else                                         hit_history.value = [hit_history(:) ; 0]; %#ok<NODEF>
          end;
        end;
        
        nTrials.value = nTrials + 1; %#ok<NODEF>
        if hit_history(length(hit_history)) == 1, nRewards.value = nRewards + 1; end;     %#ok<NODEF>
