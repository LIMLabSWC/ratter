...
switch action,
	...
	case 'init'
		...
		next_row(y, 1.5);
		next_column(x); y = 5;
		NumeditParam(obj, 'CP_duration', 0.250, x,y,'label','CP duration','TooltipString','Duration of Nose in Central Poke before Go cue starts (see Total_CP_duration)');
		...
		next_row(y);
		NumeditParam(obj, 'CenterLed_duration', 20, x,y,'label','Central LED duration','TooltipString','Duration of Center Led');
		next_row(y);
		NumeditParam(obj, 'SideLed_duration', 0.5, x,y,'label','Side LED duration','TooltipString','Duration of SideLed');
		next_row(y);
		NumeditParam(obj, 'legal_cbreak', 0, x,y, 'position', [x, y, 175 20], 'TooltipString','Time in sec for which it is ok to be outside the center port before a violation occurs.');
        ...
        next_row(y);
		NumeditParam(obj, 'settling_legal_cbreak', 0.05, x,y, 'position', [x, y, 175 20], 'TooltipString','Time in sec for which it is ok during the "SettlingIn_time" to be outside the center port before a violation occurs.');
		...
		next_row(y);
		%         NumeditParam(obj, 'wait_for_cpoke_timeout', 4, x,y,'label','Response Timeout','TooltipString','Time after NIC to wait for a side poke');
		%         next_row(y);
		

    NumeditParam(obj, 'A1_time', 3, x,y,'label','AUD1 on Time','TooltipString','Duration of first stimulus');
    next_row(y);
	NumeditParam(obj, 'A2_time', 3, x,y,'label','AUD2 On Time','TooltipString','Duration of second stimulus');
	next_row(y);
	NumeditParam(obj, 'Del_time', 2, x,y,'label','Delay Duration Time','TooltipString','Duration of delay period');
	next_row(y);
    NumeditParam(obj, 'PreStim_time', 0.05, x,y,'label','Pre-Stim NIC time','TooltipString','Time in NIC before starting the stimulus');
    next_row(y);
    NumeditParam(obj, 'time_bet_aud2_gocue', 0.05, x,y,'label','A2-GoCue time','TooltipString','time between the end of the second stimulus and the go cue ');
	next_row(y);
		...
		next_row(y);
		NumeditParam(obj,'training_stage',2,x,y,'label','Training Stage');
		next_row(y);
		ToggleParam(obj,'use_training',0,x,y,'OnString','Using Autotrain','OffString','Manual Settings');
		
        next_row(y);
		NumeditParam(obj, 'ntrial_correct_bias', 30, x, y, ...
	...
	case 'new_CP_duration', 
        if training_stage ==2
            CP_duration=PreStim_time + A1_time + A2_time + Del_time + time_bet_aud2_gocue;
        end
	...
	case 'prepare_next_trial'
		...
		switch value(training_stage)
			...
			case 2,  %  like stage 1 - now waiting time grows
                CP_duration=PreStim_time + A1_time + A2_time + Del_time + time_bet_aud2_gocue;
