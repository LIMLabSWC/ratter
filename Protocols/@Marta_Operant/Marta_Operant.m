% This protocol runs a simple operant task 
%
% Masayoshi Murakami - October 2008

% To run it:
%  newstartup
%  dispatcher('init')
%  select this protocol from pull down menu
%  and then press RUN
%

function [obj] = Marta_Operant(varargin)
...
switch action,
    ...
    case 'init'
        ...
        set(value(myfig), 'Position', [890   100   210   360]);
        ...
        my_state_colors = struct( ...
            'state_0',                  [0.2 0.2 0.2], ...
            'wait_for_cpoke',           [0.4 1 0.4],   ...
            'wait_for_apoke',           [1 1 1],   ...
            'pre_left_reward',          [0.5 0.5 1], ...
            'pre_right_reward',         [0.5 0.5 1], ...
            'left_reward',              [0 0 1], ...
            'right_reward',             [0 0 1], ...
            'check_next_trial_ready',   [0.2 0.2 0.2]);
        ...
        [x, y, ...
            ValveRight, ValveLeft, DelayToRewardLeft, DelayToRewardRight, ...
            RewardAvailPeriod, CPokeNecessary] ...
            = ParamsSection(obj, 'init', x, y);

        SoloFunctionAddVars('StateMatrixSection', 'rw_args', {}, ...
            'ro_args', {ValveRight, ValveLeft, ...
                        DelayToRewardLeft, DelayToRewardRight, ...
                        RewardAvailPeriod, CPokeNecessary});
