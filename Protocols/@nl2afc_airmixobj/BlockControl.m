...
switch action,
    case 'init',
        ...
        next_row(y);
        NumeditParam(obj, 'score_pass',85, x, y, 'label', 'acceptable score', 'labelfraction',0.5);
        next_row(y);
        MenuParam(obj, 'block_update', {'upon_score', 'upon_block','stay','interlv', 'go_probe'}, 1,x, y);
        next_row(y);
        DispParam(obj, 'n_interlv_trials_left', 0, x, y);
        next_row(y);
        MenuParam(obj, 'mixid_rept',{1, 2, 3},1 , x,y, 'position',[x y 100 20]);
        NumeditParam(obj,'repts', 100, x, y, 'position', [x+50 y 150 20],...
            'labelpos', 'left', 'TooltipString', 'Number of repeats for each mixture in the random condition');
        SoloParamHandle(obj, 'mix_repts', 'value', ones(1,length(value(mix_names)))*value(repts)); 
...
%        SoloParamHandle(obj, 'perform_overall', 'value', ones(maxtrials,4)*(-1));
  ...
  %-------- Initilize a plot monitoring rats perfomance-------------
   ...
   %     SoloParamHandle(obj, 'h2',  'value', axes('Position', [0.56, 0.52, 0.4, 0.25])); % axes
    ...
    case 'user_specify'
        % if user changed the mix_ID which is specified in OdorSection.m
    ...
    case 'rand_interlv'
        ...
        if strcmp(value(block_update),'interlv')
            ...
            mix_repts(value(mixid_rept)) = value(repts); % assign current number of repeats to the corrent mix ID
            % create a vector (pool) to be randomly permuated.
            for i = 1: length(value(mix_repts))
                rept_pool = [rept_pool ones(1, mix_repts(i))*i];
            end;
            %      rept_pool = [ones(1, mix_repts(1)) ones(1,mix_repts(2))*2];
            if length(value(mix_names)) == 2
                [C1 tgid] = min(value(mix_repts)); % the mix id of fewer repeats as the target mixture.
                [C2 ntgid] = max(value(mix_repts)); % the mix id of more repeats as the non target.
                tgt_freq = C1/sum(value(mix_repts));
                if tgt_freq > 0.3,
                    r = randperm(length(rept_pool)); rept_pool = rept_pool(r);
                    randrepeatpool.value = rept_pool;
                else
                    if tgt_freq > 0.2,
                        x = 2;
                    else
                    ...
                    d = {[],[]};
                    % if ~ismember(1,d(1)), no consecutive 2 tgid in the pool.
                    % if ~ismember(1,d(2)), only contains at most 2 tgid in the pool.
                    mixrp1= C1; mixrp2 = C2;
                    while length(rept_pool) > 100,
                        rp1 = tgt_freq * 100;
                        rp2 = (1-tgt_freq) * 100;
                        rnd_pl = [ones(1,rp1)*tgid ones(1,rp2)*ntgid];
                        d{1} = diff(find(rnd_pl == tgid)); d{2} = diff(find(d{1}==1));
                        while ismember(1,d{x}),
                            r = randperm(length(rnd_pl)); rnd_pl = rnd_pl(r);
                            d{1} = diff(find(rnd_pl == tgid)); d{2} = diff(find(d{1}==1));
                        end;
                        randrepeatpool.value = [value(randrepeatpool) rnd_pl];
                        mixrp1= mixrp1 - rp1; mixrp2 = mixrp2 - rp2;
                        rept_pool = [ones(1, mixrp1)*tgid ones(1,mixrp2)*ntgid];
                    end;
                    if ~isempty(rept_pool),
                        d{1} = diff(find(rept_pool == tgid)); d{2} = diff(find(d{1}==1));
                        while ismember(1,d{x}),
                            r = randperm(length(rept_pool)); rept_pool = rept_pool(r);
                            d{1} = diff(find(rept_pool == tgid)); d{2} = diff(find(d{1}==1));
                        end;
                        randrepeatpool.value = [value(randrepeatpool) rept_pool];
                    end;
                end;
            else
                r = randperm(length(rept_pool)); rept_pool = rept_pool(r);
    ...
    case 'update'
        n_interlv_trials_left.value = size(value(randrepeatpool),2);
        block_updated = 0;
        ...
        block(blk).mix_diff = value(curr_diff); 
            ...
            if strcmp(value(block_update), 'upon_score')
             %   WaterDelivery.value = 3;
                    ...
                    if block(blk).score >= score_pass
                        ...
                        % the first one again.
                        if block(blk+1).mix_id> length(value(mix_names))
