...
%% Stage 7: Continue Centre Poke Training with variable pre-stim time and variable delay time (both within a range).
% This variable time is introduced so that rat doesn't learn the expectancy
...
switch action
    ...
    case 'prepare_next_trial'
        ...
        sone_sound_duration     = value(A1_time); % SoundManagerSection(obj, 'get_sound_duration', 'SOneSound');
        ...
        SoftPokeStayInterface(obj, 'set', 'soft_cp', 'Duration', CP_duration - SettlingIn_time,'Grace', legal_cbreak); % used in later stages, after stage 3
        ...
        sma = add_scheduled_wave(sma, 'name', 'stimA1', 'preamble', PreStim_time, ...
            'sustain', sone_sound_duration, 'sound_trig', sone_sound_id); % to play a sound before Go Cue
        ...
        switch value(training_stage)
            ...
            case 1  % LEARNING THE REWARD SOUND ASSOCIATION -LEFT OR RIGHT LED ON -> POKE -> SOUND+REWARD GIVEN
                ...
                sma = add_state(sma,'name','hit_state','self_timer',0.01,...
                    'output_actions', {'DOut', SideLight; 'SchedWaveTrig','reward_delivery + Go_Cue'},...
            ...
            case {4,5,6,7} % STAGE 4 - LEARN TO NOSE POKE BEYOND SETTLING TIME WITH THE INTRODUCTION OF VIOLATION, THIS IS UNTIL CP = 1 SEC
                ...
                %     'output_actions', {'DOut', center1led,'SchedWaveTrig',  settling_period}, ...
                %     'input_to_statechange', {'Cout','intermid_poke_out_state','Tup','soft_cp'});

                % Before progressing check if its still centre poking or pokes within legal c_break other wise its a violation

                % This state is considering the poke is out at the end of settling time / at the start of this state

                % sma = add_state(sma,'name','intermid_poke_out_state','self_timer',SettlingIn_time, ...
                %     'output_actions', {'DOut', center1led}, ...
                %     'input_to_statechange', {'settling_period_In','legal_poke_start_state','Cin','intermid_poke_in_state','Tup','legal_poke_start_state',...
                %     'Rin',  'violation_state','Rout', 'violation_state', 'Lin', 'violation_state','Lout', 'violation_state'});

                % The other case is when the nose is still in then go directly to soft_cp

                % sma = add_state(sma,'name','intermid_poke_in_state','self_timer',SettlingIn_time, ...
                %     'output_actions', {'DOut', center1led}, ...
                %     'input_to_statechange', {'settling_period_In','soft_cp', 'Cout','intermid_poke_out_state','Tup','soft_cp',...
                %     'Rin',  'violation_state','Rout', 'violation_state', 'Lin', 'violation_state','Lout', 'violation_state'});
                ...
                % Instead using Intermediate State
                ...
                sma = add_state(sma,'name','legal_poke_start_state','self_timer',legal_cbreak, ...
                    'output_actions', {'DOut', center1led}, ...
                ...
                if value(training_stage) == 4
                    ...
                    sma = SoftPokeStayInterface(obj, 'add_sma_states', 'soft_cp', ...
                        {'success_exitstate_name', 'legal_poke_end_state'},{'abort_exitstate_name', 'violation_state'}, ......
                        'pokeid', 'C','ConstantDOut', center1led, 'triggertime', 0.0001);
                    ...
                    sma = SoftPokeStayInterface(obj, 'add_sma_states', 'soft_cp', ...
                        {'success_exitstate_name', 'legal_poke_end_state'},{'abort_exitstate_name', 'violation_state'}, ......
                        'pokeid', 'C','ConstantDOut', center1led, 'triggertime', 0.0001,'InitialSchedWaveTrig', stimA1);
                ...
                sma = add_state(sma,'name','legal_poke_end_state','self_timer',legal_cbreak, ...
                    'output_actions', {'DOut', center1led}, ...
                ...
                sma = add_state(sma,'name','second_hit_state','self_timer',RewardCollection_duration,...
                    'output_actions',{'DOut', SideLight},...
                    'input_to_statechange',{'reward_collection_dur_In', 'timeout_state'; 'Tup','punish_state'; HitEvent,'hit_state'});
               ...
               % For Violations
                ...
                sma = add_state(sma,'name','violation_state','self_timer',viol_snd_duration,...
                    'output_actions',{'SoundOut',viol_sound_id; 'DOut', center1led; 'SchedWaveTrig', '-Go_Cue'},...
                    'input_to_statechange', {'Tup', 'current_state+1'});
