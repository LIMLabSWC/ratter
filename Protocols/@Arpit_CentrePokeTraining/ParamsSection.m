...
switch action
	...
	case 'init'
		...
		next_column(x); y = 5;
        % Reward Collection
		NumeditParam(obj, 'RewardCollection_duration', 10, x,y,'label','RewardCollection_duration','TooltipString','Wait until rat collects the reward else a timeout');
		next_row(y);
		...
		next_row(y);
        NumeditParam(obj, 'A1_time_Min', 0.4, x,y,'label','Min time for AUD1','TooltipString','Min value to select the Duration of fixed stimulus');
        set_callback(A1_time_Min, {mfilename, 'new_CP_duration'});
        next_row(y);
        NumeditParam(obj, 'A1_time', 0.4, x,y,'label','AUD1 on Time','TooltipString','Actual Duration of fixed stimulus');
        next_row(y);
        set_callback(A1_time, {mfilename, 'new_CP_duration'});
        next_row(y);
        NumeditParam(obj, 'A1_time_Max', 0.4, x,y,'label','Max time for AUD1','TooltipString','Max value to select the Duration of fixed stimulus');
        set_callback(A1_time_Max, {mfilename, 'new_CP_duration'});
        next_row(y);
        NumeditParam(obj, 'PreStim_time_Min', 0.20, x,y,'label','Min time for Pre-Stim','TooltipString','Min Time in NIC before starting the stimulus');
        set_callback(PreStim_time_Min, {mfilename, 'new_CP_duration'});
        next_row(y);
        NumeditParam(obj, 'PreStim_time', 0.20, x,y,'label','Pre-Stim NIC time','TooltipString','Actual Time in NIC before starting the stimulus');
        set_callback(PreStim_time, {mfilename, 'new_CP_duration'});
        next_row(y);
        NumeditParam(obj, 'PreStim_time_Max', 0.40, x,y,'label','Max time for Pre-Stim','TooltipString','Max Time in NIC before starting the stimulus');
		...
		next_row(y);
		NumeditParam(obj, 'timeout_iti', 5, x,y,'label','Error Timeout','TooltipString','ITI on timeout trials');
		next_row(y);
		...
		SoloFunctionAddVars('Arpit_CentrePokeTrainingSMA', 'ro_args', ...
			...
			'secondhit_delay';'timeout_iti';'violation_iti'});
	...
	case 'new_CP_duration' 
        ...
        if random_PreStim_time == 1 && (PreStim_time.value < value(PreStim_time_Min) || PreStim_time.value > value(PreStim_time_Max))
            PreStim_time.value = value(PreStim_time_Min);
        else
            PreStim_time.value = value(PreStim_time);
        end

        if random_A1_time == 1 && (A1_time.value < value(A1_time_Min) || A1_time.value > value(A1_time_Max))
            A1_time.value = value(A1_time_Min);
        else
            A1_time.value = value(A1_time);
        end

        if random_time_bet_aud1_gocue == 1 && (time_bet_aud1_gocue.value < value(time_bet_aud1_gocue_Min) || time_bet_aud1_gocue.value > value(time_bet_aud1_gocue_Max))
            ...
            case {3,4} % Centre poke without the A1_Stim but has violation in 4
                ...
                case {5,6,7} % 
                   ...
                   if n_done_trials <1 && warmup_on ==1
       				...
       				CP_duration.value=settling_time + A1_time + PreStim_time + time_bet_aud1_gocue;
                   end
                   Total_CP_duration.value = CP_duration + time_go_cue; %#ok<*NASGU>
