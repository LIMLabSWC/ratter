function out = tonesamp(varargin)
...
switch action
    ...
    case 'init'
        ...
        OdorCh=[Blnk Blnk Blnk Blnk Blnk Blnk  Blnk Blnk  Blnk Blnk Ch_A Ch_B Blnk];
        Freq  =[500 1000 2000 5000 7000 13000 15000 17000 23000 -1   500  500  NaN];  %Hz
        Dur   =[200  200  200  200  200   200   200   200   200 200  500  500    1];  %msec
        SPL   =[-10   65   65   65   65    65    65    65    65  35  -10  -10   70];  %Max=PPdB SPL
        Str   ={'silence','1k 65dB 200ms','2k 65dB 200ms','5k 65dB 200ms','7k 65dB 200ms','13k 65dB 200ms',...
                '15k 65dB 200ms','17k 65dB 200ms','23k 65dB 200ms','wn 35dB atn 200ms','odor A','odor B','Wav file'}; 
        ...
        rpbox('loadrp3stereosound',{beep{GetParam(me,'Tone_Select')},vp_sound{GetParam(me,'VP_Signal')}});
...
%         rpbox('send_matrix',state_transition_matrix(NextParam));
    ...
    case 'update'
        ...
        for i=1:size(Event,1)
            ...
            if (Event(i,1)==5 | Event(i,1)==6)        %Valid poke
                message(me,'Valid Poke','cyan');                
            elseif Event(i,1)==7    %time out
                message(me,'Time Out');
            end
        end
    ...
    case 'tone_select'
        ...
        n_tones=length(Freq);
        if GetParam(me,'Tone_Select')==n_tones
            %             RPFs=5e7/2^10;
            ...
            Str=GetParam(me,'Tone_Select','list');
            Dur=round(length(sRPS)/RPFs*1000);
            Str{end}=[filename(1:end-4) ' ' num2str(Dur) 'ms'];            
            SetParam(me,'Tone_Select','list',Str);
        else
        end
        rpbox('loadrp3stereosound',{beep{GetParam(me,'Tone_Select')}});
...
%   2    2   2    2   2    2    3     wvd  wpt tns+2;  % State 2 "Valid Poke ==> Water!!! :)"
    2    2   2    2   2    2    3     wvd  wpt     0;  % State 2 "Valid Poke ==> Water!!! :)"
...
if bitget(WaterPort,1)
    LinS=5;
else
    LinS=7;
end

if bitget(WaterPort,2)
    RinS=6;
else
    RinS=7;
end
...
%  Cin Cout Lin   Lout Rin   Rout TimeUp        Timer DIO  AO  
    1    0   0      0   0      0    0           180   0      0;  % State 0 "Pre-State"
    1    7   1      1   1      1    2           .01   0      0;  % State 1 "Center Poke in, before tone on"
    2    7   2      2   2      2    3+(ltd==0)  vpd  odr tns+0;  % State 2 "Tone On"
%   3    3  LinS    0  RinS    0    4           ltd  odr tns+2;  % State 3 "pre- Center Poke out"
%   4    4  LinS    0  RinS    0    0           rad   0  tns+2;  % State 4 "Reward Avaiable Dur"
    3    3  LinS    0  RinS    0    4           ltd  odr     0;  % State 3 "pre- Center Poke out"
    4    4  LinS    0  RinS    0    0           rad   0      0;  % State 4 "Reward Avaiable Dur"
    5    5   5      5   5      5   35           wvd  wpt      0;  % State 5 "Valid Poke ==> Water!!! :)"
    6    6   6      6   6      6   35           wvd  wpt     0;  % State 6 "Valid Poke ==> Water!!! :)"    
    7    7   7      7   7      7    0           tmo   0      0;];% State 7 "TimeOut ==> House Light "
...
case 3
    ValidTonePokeDur(1:ceil(tr/6))  = 0.1;
    ValidTonePokeDur(ceil(tr/6)+1:ceil(tr/6)*2) = 0.2;
    ValidTonePokeDur(ceil(tr/6)*2+1:ceil(tr/6)*3) = 0.3;
    ValidTonePokeDur(ceil(tr/6)*3+1:tr) = 0.4;
    RewardAvailDur(1:tr)  = 3;
case 4
    ValidTonePokeDur(1:ceil(tr/6)*2)  = 0.3;
    ValidTonePokeDur(ceil(tr/6)*2+1:tr) = 0.4;
...
for tn=1:n_tones-1
    ...
    beep{tn}  = 1 * makebeep(RPFs, ToneAttenuation_adj ,Freq(tn), Dur,3);
end
SetParam(me,'beep',beep);
out=beep;
