function [obj] = locsamp_childobj(a)
...
% ------ Definitions for methods that are pre-defined by the superclass, but may be overloaded by the child for customisation.

% [x, y] = InitSaving(x, y, super);				next_row(y);
% [x, y, RightWValve, LeftWValve] = InitWaterValves(x, y, super);	next_row(y);
...
% B. Copied straight out of locsamp_childobj -------------------------------
...
ro_args = {'n_done_trials', 'n_started_trials', 'maxtrials', 'chord_sound_len'};
...
%next_row(y, 0.5);
x = 1;
HeaderParam(obj, 'prot_title', 'Locsamp_Child: A model child protocol class', ...
    x, (fig_position(2)+fig_position(4))-50, ...
...
SoloFunction('make_and_upload_state_matrix', ...
    ...
    'TimeOutSound', 'TimeOutLength', 'TimeOutReinitPenalty', 'BadBoySound', ...
     });
