...
switch action,
  ...
  case 'init'
    ...
    set(value(myfig), 'Position', [300   200   650   400]);
    ...
    MenuParam(obj, 'Task_Phase', {'Lick4Sound', 'AllMatches', 'Match2Sample', 'NonMatch2Samp'}, 'Match2Sample', xpos, ypos); next_row(ypos);
    ...
    Matches =  ceil(2.*rand(value(MaxTrials),1)); 
    Mismatches = ceil(2.*rand(value(MaxTrials),1)); 
    for x = 1:MaxTrials
    ...
    SoundManagerSection(obj, 'declare_new_sound', 'Squeak', [0]);  
...
% cf = 			5000.000;                  % carrier frequency (Hz)
% %sf = 200000;                 % sample frequency (Hz)
% d = 0.25;                    % duration (s)
% n = sf * d;                 % number of samples
% s = (0:n) / sf;             % sound data preparation
% Squeak = .02*sin(2 * pi * cf * s);   % sinusoidal modulation
% %     Squeak = importdata('Squeak.Mat');
...
%     SoundManagerSection(obj, 'set_sound', 'Squeak', Squeak);
  ...
  case 'prepare_next_trial'
   ...
   if(n_done_trials>1)
       if  ~(strcmp(value(Task_Phase),'NonMatch2Samp')) 
