...
%
% dispatcher('close_protocol'); dispatcher('set_protocol','saja_expectation');
%
...
function [obj] = saja_expectation(varargin)
...
switch action,
  ...
  case 'init'
    ...
    % ----------------- Psychometric test -----------------
    MenuParam(obj, 'PsychCurveMode', {'off' 'on'}, 'off', x, y,...
              'label','Psych Curve');
    next_row(y,1.5);
    ...
    SoloFunctionAddVars('SoundsSection', 'rw_args',...
                        {'RelevantSide','MaxTrials','PsychCurveMode'});
