...
%        Cin    Cout    Lin    Lout     Rin     Rout    Tup    Timer    Dout  odor  Aout
...
switch WaterDelivery,
    ...
    case 'probe'
        if     side==left, lpkA = LrdS; rpkA = RrdS
        elseif side==1-left, rpkA = RrdS; lpkA = RrdS;
        end;
        if rand > .5
            deliver_water.value = 1;
        else
...
vid = value(active_valve); % odor valve ID
auxai = 1; % trigger auxai high for measurement of vavle latency

% Cin    Cout   Lin    Lout    Rin    Rout   Tup    Timer   Dout  odor Aout
stm = [stm ; ...
   b+1     b      b      b       b      b   ITI_state+1  15   0    0    0 ; ...    %b: wait for c_poke
   b+1     b      b+1    b+1     b+1    b+1  b+2    rvd       0    0    0 ; ...    %b+1 randam valve delay
   b+2     b      b+2    b+2     b+2    b+2  b+3    fvd       0   vid   0;  ...    %b+2: odor travel time
   b+3     b      b+3    b+3     b+3    b+3  b+4    vst       fv   vid   0 ; ...    %b+3: valide odor poke
   b+4     b+5    b+4    b+4     b+4    b+4  b+5    1         fv   vid   0 ; ...    %b+4: time before poke out
    ];
if ~strcmpi(value(WaterDelivery), 'direct')
    stm = [stm; ...
        b+5   b+5    lpkA   b+5    rpkA    b+5   ITI_state+1   avail  0   0   0 ; ...    %b+4: WpkS wait for ans poke
        ];
else % go directly to water delivery
    stm = [stm; ...
        lpkA    lpkA    lpkA    lpkA    lpkA    lpkA    lpkA    0.02  0  0   0       ;...
        ];
end;

stm = [stm; ...
    LrwS  LrwS  LrwS    LrwS   LrwS    LrwS  ITI_state  lwpt    lvid   0    0     ; ...
    RrwS  RrwS  RrwS    RrwS   RrwS    RrwS  ITI_state  rwpt    rvid   0    0     ; ...
    LddS  LddS  LddS    LddS   LddS    LddS  ITI_state  lwpt    lvid   0    0     ; ...
    RddS  RddS  RddS    RddS   RddS    RddS  ITI_state  rwpt    rvid   0    0 ];


% and finally add the ITI state
iti = ITI_state; iti_wrong = ITI_state + 1;
stm = [stm ; ...
    iti   iti   iti     iti     iti     iti     35 iti_length   0   0    0; ...  % benign ITI state
    iti+1 iti+1 iti+1   iti+1   iti+1   iti+1   35 iti_length   0   0    0; ...  % ITI where miss trials end up
    ];
...
stm = [stm;...
    LrdS  LrdS  LrdS    LrdS   LrdS    LrdS  ITI_state  lwpt    lvid   0    0     ; ...
    RrdS  RrdS  RrdS    RrdS   RrdS    RrdS  ITI_state  rwpt    rvid   0    0 ];
...
fsm = SetOutputRouting(fsm, {struct('type', 'dout', 'data', '0-15');...
       struct('type', 'tcp', 'data', [value(OLF_IP) ':3336:SET BANK ODOR Bank' num2str(value(active_bank)) ' %v']);...
