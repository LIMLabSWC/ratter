% [x, y] = SoundControlsSection(obj, action, varargin)

function [x, y] = SoundControlsSection(obj, action, varargin)
...
%     SoloParamHandle(obj, 'scsets');
%     scsets.value.width    = width;
%     scsets.value.ShowVol  = ShowVol;  %#ok<NODEF>
%     scsets.value.ShowFreq = ShowFreq; %#ok<NODEF>
%     scsets.value.ShowDur  = ShowDur;  %#ok<NODEF>
%     scsets.value.ShowLoop = ShowLoop; %#ok<NODEF>
%     scsets.value.ShowSide = ShowSide; %#ok<NODEF>
%     scsets.value.ShowBal  = ShowBal;  %#ok<NODEF>
%     scsets.value.vol      = setvol;
%     scsets.value.freq     = setfreq;
%     scsets.value.dur      = setdur;
%     scsets.value.side     = setside;
%     scsets.value.bal      = setbal;
%     scsets.value.loop     = setloop;
%     scsets.value.name     = name;
    ...
    if ShowSide == 1, MenuParam(obj, 'Side', {'Left', 'Center', 'Right', 'Other'}, setside, x, y, 'position', [x y width 20]); %#ok<ALIGN>
        ...
        set_callback(Side, {mfilename, 'set_side'}); next_row(y);
    end;
...
case 'set_volume'
        SoundInterface(obj, 'set', value(sname), 'Vol',   value(Vol));
...
case 'set_frequency'
        SoundInterface(obj, 'set', value(sname), 'Freq1', value(Freq));
...
case 'set_duration'
        SoundInterface(obj, 'set', value(sname), 'Dur1',   value(Dur));
...
case 'set_balance'
        SoundInterface(obj, 'set', value(sname), 'Bal', value(Bal));
...
case 'set_loop'
        SoundInterface(obj, 'set', value(sname), 'Loop',  value(Loop));
...
%       ShowSide = scsets.ShowSide;
      ...
      if ShowVol  == 1, Vol.value  = SoundInterface(obj, 'get', value(sname), 'Vol'  ); end
