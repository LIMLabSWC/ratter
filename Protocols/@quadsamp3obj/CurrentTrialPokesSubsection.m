...
%
  ...
  % 
  % 'leftover_sound',  [0.2 0.2 0.99],   ...  
state_colors = struct( ...
  'wait_for_cpoke',  'w',              ...  
  'leftover_sound',  [0.6 0.6 0.8],   ...  
  'wait_for_apoke',  [0.7 0.7 0.95],   ...  
  'left_reward',     [0.7 1 0.7],    ...
  'right_reward',    [0.7 1 0.7],    ...
  'drink_time',      [0.4 0.8 0.4],    ...
...
plottables = fieldnames(state_colors);
...
switch action,
 case 'init',  % ---------- CASE INIT ----------
   ...
   set_callback(CurrentTrialPokes, {mfilename, 'view'; mfilename, 'redraw'});
   ...
   SoloParamHandle(obj, 'histhandles', 'value', ...
                   hist_cpoke_stats(cpokestats, 'handles', [], ...
                                    'custom_colors', state_colors, ...
 ...
 case 'redraw', % ------- CASE REDRAW
   ...
   if strcmp(value(CurrentTrialPokes), 'view'),
      if n_started_trials >= 1,
         ...
         hist_cpoke_stats(cpokestats, 'handles', value(histhandles), ...
                          'custom_colors', state_colors, ...
                          'plottables',    plottables);
         drawnow;
      end;
