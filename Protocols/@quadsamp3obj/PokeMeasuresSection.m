...
switch action,
 case 'init',  % ---------- CASE INIT ----------
  ...
  SoloParamHandle(obj, 'h2', 'saveable', 0, ...
                  'value', axes('Position', [0.08 0.58 0.145 0.12]));
 ...
 case 'update_counts',  % ---------- CASE UPDATE_COUNTS ---------
  ...
  nevs            = GetEventCounter(sm);
  new_events      = GetEvents(sm, value(ProcessedEventCouner)+1, nevs);
  ProcessedEventCouner.value = nevs;
  
  Event = zeros(2*size(new_events,1), 3);
  for i=1:size(new_events,1),
     Event(2*i-1,1) = new_events(i,1);
     Event(2*i-1,2) = log2(new_events(i,2))+1;
     Event(2*i-1,3) = new_events(i,3);

     Event(2*i,1) = new_events(i,4);
     Event(2*i,2) = 0;
     Event(2*i,3) = new_events(i,3);
  end;
 ...
 case 'update_plot', % --------- CASE UPDATE_PLOT ------- 
   ...
   if length(u)>0,
      ...
      if isfield(RealTimeStates, 'leftover_sound')
         RealTimeStates.wait_for_apoke = ...
             colvec(RealTimeStates.wait_for_apoke);
         RealTimeStates.leftover_sound = ...
             colvec(RealTimeStates.leftover_sound);
         red_u = find(ismember(CPokes.poke_state(u), ...
                               [RealTimeStates.wait_for_apoke ; ...
                             RealTimeStates.leftover_sound]));
      else
         red_u = find(ismember(CPokes.poke_state(u), ...
