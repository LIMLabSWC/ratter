function [obj] = multipokesobj(name)
...
% Default object:
obj = class(struct, mfilename, oldpokesplot);
...
if nargin~=1 || ~ischar(name),    % <~> isstr to ischar, matlab recommen.
...
set(value(myfig), 'Position', [485   144   650   880]);
...
NumeditParam(obj, 'MaxTrials', 150, x, y); next_row(y);
NumeditParam(obj, 'MaxMins', 75, x, y); next_row(y);
SoloParamHandle(obj, 'protocol_start_time', 'value', []);
SoloFunctionAddVars('make_and_upload_state_matrix', 'ro_args', {'MaxTrials','MaxMins', 'protocol_start_time'});
SoloFunctionAddVars('state35', 'rw_args', 'protocol_start_time');
next_row(y);
...
HeaderParam(obj, 'prot_title', 'Multipokes', ...
