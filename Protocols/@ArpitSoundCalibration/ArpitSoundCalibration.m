...
function calculate_soundlevel

% --- Configuration ---
serialPort = '/dev/ttyACM0'; % Replace with the actual serial port of your Raspberry Pi
baudRate = 9600; % Or your desired baud rate
matlabOutputValues = 0:10:100; % Example range of MATLAB output values
calibrationData = [];
...
try
    ...
    fopen(s);
    disp(['Opened serial port: ', serialPort, ' at ', num2str(baudRate), ' bps']);

    % --- Calibration Loop ---
    for i = 1:length(matlabOutputValues)
        outputValue = matlabOutputValues(i);
        disp(['Sending MATLAB output value: ', num2str(outputValue)]);

        % --- Send command to set speaker level (replace with your actual command) ---
        setSpeakerLevel(outputValue);

        % --- Send command to Raspberry Pi to measure SPL ---
        fprintf(s, 'measure_spl\n'); % Send command over serial

        % --- Receive SPL value from Raspberry Pi ---
        receivedData = fgetl(s);
        if ~isempty(receivedData)
            splValue = str2double(receivedData);
            if ~isnan(splValue)
                disp(['Received SPL: ', num2str(splValue), ' dB']);
                calibrationData = [calibrationData; outputValue, splValue];
                pause(2); % Allow time for sound to stabilize and measurement
            else
                warning('Received non-numeric SPL value.');
            end
        else
            warning('No data received from Raspberry Pi.');
        end
    end
    ...
    disp('Calibration Data (MATLAB Value, dB SPL):');
    disp(calibrationData);

    % --- Create Calibration Mapping (Example: Linear Fit) ---
    p = polyfit(calibrationData(:,1), calibrationData(:,2), 1);
    disp('Calibration Polynomial (p(1) * MATLAB_Value + p(2)):');
    disp(p);
    % Now you can use this polynomial 'p' to estimate dB SPL from your MATLAB values
    % estimatedSPL = polyval(p, yourMatlabValue);
...
% --- Your function to control the speaker level in MATLAB ---
function setSpeakerLevel(value)
    % Replace this with your actual commands or functions to control the speaker.
