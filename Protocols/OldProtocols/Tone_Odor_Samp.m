...
switch action
    ...
    case 'init'
        ...
        OdorCh =[Blnk Ch_A Ch_B Ch_A+Ch_B];
        OdorStr={'no odor','odor A','odor B','OdorA+B'}; 
        
        Freq  =[500 1000 2000 5000 7000 13000 15000 17000 23000 -1 ];  %Hz
        Dur   =[200  200  200  200  200   200   200   200   200 200];  %msec
        SPL   =[-10   65   65   65   65    65    65    65    65  35];  %Max=PPdB SPL
        Str   ={'silence','1k 65dB 200ms','2k 65dB 200ms','5k 65dB 200ms','7k 65dB 200ms','13k 65dB 200ms',...
                '15k 65dB 200ms','17k 65dB 200ms','23k 65dB 200ms','wn 35dB atn 200ms'}; 
        ...
        InitParam(me,'Odor_Select','ui','popupmenu','list',OdorStr,'value',1,'user',1,'pos',[h n*vs hs*.85 vs]); 
        SetParamUI(me,'Odor_Select','label','Select Odor');
        InitParam(me,'Tone_Delay','ui','edit','value',0.001,'range',[0.0005 .5],'pos',[h+hs*1.8 n*vs hs*.6 vs]); n=n+1;
        ...
        SetParamUI(me,'Tone_Select','label','Select Tone');
        ...
        set(fig,'pos',[140 461-n*vs hs*3.2 (n+17)*vs],'visible','on');
        plot_schedule;
...
%         rpbox('send_matrix',state_transition_matrix(NextParam));
    ...
    case {'waterport','watervalvedur' 'odor_select'}
        rpbox('send_matrix',state_transition_matrix(NextParam));
...
och=och(GetParam(me,'Odor_Select'));
odr=och + (2^2)*(och>0)
dly=GetParam(me,'Tone_Delay');
...
%  Cin Cout Lin   Lout Rin   Rout TimeUp        Timer DIO  AO  
    1    0   0      0   0      0    0           180   0      0;  % State 0 "Pre-State"
    1    7   1      1   1      1    2           dly  odr     0;  % State 1 "Center Poke in, before tone on"
    ...
    5    5   5      5   5      5   35           wvd   1      0;  % State 5 "Left Valid Poke ==> Water!!! :)"
    6    6   6      6   6      6   35           wvd   2      0;  % State 6 "Right Valid Poke ==> Water!!! :)"
    7    7   7      7   7      7    0           tmo   0      0;];% State 7 "TimeOut  "
...
for tn=1:n_tones
    ...
    beep{tn}  = 1 * makebeep(50e6/1024, ToneAttenuation_adj ,Freq(tn), Dur,3);
end
