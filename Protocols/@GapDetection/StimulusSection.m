...
%     next_row(y);
    ...
    set_callback_on_load(numClass, 10); %#ok<NODEF>
    set_callback(numClass, {mfilename, 'numClass'});
    numClass.value = 10; callback(numClass);
  ...
  case 'prepare_next_trial' 
    ...
    if strcmp(Rule,'S2>S1 Left')
        if strcmp(ThisTrial, 'LEFT')
            ...
            %gaussian
            if (flag_stimdist=='wide_gauss')||(flag_stimdist=='narrow_gauss')
                if(flag_stimdist=='wide_gauss')
                    mu = 5;
                    sigma = 1.5;
                else
                    mu = 5;
                ...
                bag = randsample(1:value(numClass),max_trials,true,prob) %ask about number of samples
                
            %bimodal
            elseif (flag_stimdist=='bimodal')
                mu = 5;
                sigma = 1.5;
                x = linspace(1,value(numClass),value(numClass));
                xU = x + 0.5; xL = x - 0.5;
                ...
                prob = prob / prob.sum() 
                nums1 =  randsample(1:value(numClass),max_trials,true,prob)
                bag = horzcat(nums,nums1);
            end
%             for cc=1:value(numClass)+(numClass-1)*value(midclass_pairs)
