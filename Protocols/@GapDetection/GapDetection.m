% Gap Detection Protocol (2 poke) in BControl
% Sharbat, Oct 2019

function [obj] = GapDetection(varargin)
...
switch action,
   ...
   case 'init'
    ...
    SoloParamHandle(obj, 'post_DelComp_protocol', 'value', '', 'save_with_settings', 1); % change for gapdetection if needed
                                                                            % returns '' for init
    SoloParamHandle(obj, 'post_DelComp_settings_filename', 'value', '', 'save_with_settings', 1); % change for gapdetection if needed
                                                                            % returns '' for init
    ...
    % Adding Task Feature values from Bpod code in Solo
    
    SoloParamHandle(obj, 'TrainingLevel', 'value', 2);
    DeclareGlobals(obj, 'ro_args', {'TrainingLevel'});
    SoloFunctionAddVars('SideSection', 'rw_args', 'TrainingLevel');
    
    SoloParamHandle(obj, 'BG_NoiseType', 'value', []);
    DeclareGlobals(obj, 'ro_args', {'timeout_history'});
    SoloFunctionAddVars('SideSection', 'rw_args', 'timeout_history');
