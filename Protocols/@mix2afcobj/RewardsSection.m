...
switch action,
    case 'init',
        ...
        % Make separate figure for performance history
        MenuParam(obj, 'performance_history', {'hide', 'view'}, 1, x, y);
        next_row(y);

        SoloParamHandle(obj, 'performance_history_fig', 'value', figure, 'saveable', 0);

        set(value(performance_history_fig), 'Visible', 'off', 'MenuBar', 'none', 'Name', 'Performance history',...
    ...
    case 'update',
        ...
        if ~isempty(u)
            if (evs(u(1),2)==3  &  side_list(n_done_trials)==1) % left
                ...
                hh(n_done_trials) = 1;
            elseif (evs(u(1),2)==5  &  side_list(n_done_trials)==0), % right
                rew=value(RightRewards); rew(n_done_trials)=1; RightRewards.value=rew;
                hh(n_done_trials) = 1;
            elseif (((evs(u(1),2)==3) | (evs(u(1),2)==5))  &  side_list(n_done_trials)==3), % rewarded at both L and R
                rew=value(RightRewards); rew(n_done_trials)=1; RightRewards.value=rew;
                hh(n_done_trials) = 1;
        ...
        text(init_x + (1 * hor_buff), init_y, '% Odor A', 'FontSize', title_size, 'HorizontalAlignment', 'Center');
...
%             eval(strcat('tf = [tf mix_trials_fraction', num2str(ind), '];'));
%         end
        tf = value(overall_trials_fraction); % SPH from SidesSection
        ...
        for ind = presented_odors
            ...
            text(xpos, ypos, num2str(eval(strcat('odor_A_percent', num2str(ind)))), 'HorizontalAlignment', 'Center'); %SPH from OdorSection
