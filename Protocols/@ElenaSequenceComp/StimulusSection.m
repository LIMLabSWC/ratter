...
%     next_row(y);
    ...
    next_row(y);
    NumeditParam(obj,'s2_s1_ratio',1.9,x,y,'label','s2_s1_ratio','TooltipString','Intensity index i.e. Ind=(S1-S2)/(S1+S2)');
    next_row(y);
...
%      end
     ...
     if strcmp(Rule,'SeqA Left')
        if strcmp(ThisTrial, 'LEFT')
            ...
            seq_ix = randi([1 size(value(sequence_a),1)],1,1);
            ...
            seq_ix = randi([1 size(value(sequence_b),1)],1,1);
            thissequence.value = value(sequence_b(seq_ix,:));
            thisclass(n_done_trials+1)=seq_ix+size(value(sequence_a),1);  %b
            %thissequence.value = value(sequences(thisclass(n_done_trials+1),:));
        end 
