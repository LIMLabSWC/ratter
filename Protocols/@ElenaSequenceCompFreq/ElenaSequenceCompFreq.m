% ElenaSequenceComp
% Elena Menichini, Jan 2020

function [obj] = ElenaSequenceCompFreq(varargin)
...
switch action,
   ...
   case 'init'
    ...
    %pair_history changed to sequence_history (from AthenaDelayComp)
    SoloParamHandle(obj, 'sequence_history', 'value', []);    
    DeclareGlobals(obj, 'ro_args', {'sequence_history'});
    SoloFunctionAddVars('StimulusSection', 'rw_args', 'sequence_history');
    ...
    % colors to plot with:
    my_state_colors = ElenaSMA(obj, 'get_state_colors');
...
%    [x, y] = PlayStimuli(obj,'init',x,y);
   ...
   case 'prepare_next_trial'
	...
	% trial was a violation or not
       ...
       [sma, prepare_next_trial_states] = ElenaSMA(obj, 'prepare_next_trial');
   ...
   case 'pre_saving_settings'
    ...
    [sequence_a,sequence_b,sequences] = StimulusSection(obj,'get_sequences');  %%
    [stim1dur stim2dur stim3dur] = SideSection(obj,'get_stimdur_history'); %%
    ...
    pd.sequence_a=sequence_a(:); 
    pd.sequence_b=sequence_b(:); 
    pd.sequences=sequences(:);  
    pd.stim1dur=stim1dur(:);  
    pd.stim2dur=stim2dur(:);  
    pd.stim3dur=stim3dur(:);  
