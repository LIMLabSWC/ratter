...
%     next_row(y);
    ...
    next_row(y);
    NumeditParam(obj,'f2_f1_ratio',2.8,x,y,'label','f2_f1_ratio','TooltipString','Frequency index i.e. Ind=(S1-S2)/(S1+S2)');
    next_row(y);
    ...
    next_row(y);
    NumeditParam(obj,'probcatch',0,x,y,'label','probcatch','TooltipString','probability of having a catch stimulus as the stimulus at each trial');
    next_row(y);   
    y=5;
    next_column(x);
  ...
  case 'prepare_next_trial' 
    ...
    if value(A3) == value(A1) & ~isempty(AUD1)
        SoundManagerSection(obj, 'set_sound', 'StimAUD3', [AUD1';  AUD1'])
    elseif value(A3) == value(A2) & ~isempty(AUD2)
...
%     S2 = S2(1);
    ...
    sequence_a.value = sequences_all([1:size(sequences_all,1)/2],:);
    sequence_b.value = sequences_all([size(sequences_all,1)/2+1:end],:);
    ...
    Indf=(value(f2_f1_ratio-1))/(1+value(f2_f1_ratio));
    F1_up =[F1_min];
    for s = 1:levels-1
        F1_up = [F1_up F1_up(s)*(1+Indf)/(1-Indf)];
    end    
    F1_up = round(F1_up*10)/10;
...
%     F2 = F2(1);
    ...
    sequence_freq_a.value = sequencesf_all([1:size(sequencesf_all,1)/2],:);
    sequence_freq_b.value = sequencesf_all([size(sequencesf_all,1)/2+1:end],:);
...
%      end
    if frequency_categorization
     ...
     if strcmp(Rule,'SeqA Left')
        if strcmp(ThisTrial, 'LEFT')
            if rand(1)<=probcatch
                seq_ix = 2;%randi([1 size(seq_a,1)],1,1);
            else
                seq_ix = 1;
            end
            ...
            if rand(1)<=probcatch
                seq_ix = 2;%randi([1 size(seq_b,1)],1,1);
            else
                seq_ix = 1;
            end
    ...
    cla(value(ax))
    xd=repmat([1:1:3],size(value(sequences),1),1);
    %yd=log(value(sequences));
    axes(value(ax));
