...
RddS = pstart + 18;
ITIst = pstart+ 20;
ExtraITIst = pstart+ 21;


RealTimeStates.wait_for_cpoke=WcS;      % 40
RealTimeStates.rand_delay = randtS;    
RealTimeStates.pre_chord = pgS;    
RealTimeStates.stop_clicks = offcliS;    
RealTimeStates.chord=sndS;         
...
RealTimeStates.hit_state = ITIst;  
RealTimeStates.extra_iti = ExtraITIst; 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     % Cin    Cout    Lin   Lout   Rin   Rout    Tup    Timer   Dout    Aout
stm(WcS+1,:) = [...
        pgS   WcS    WcS    WcS   WcS    WcS    WcS     100      0      -4  ; ...   % Wait for center poke
             ];

% Delay before clicks. The clicks have 100ms of silence before train,
% so to get between 200ms and 600ms we do:
RandDelay = 0.2*rand(1) + 0.3;          % Random delay before clicks. 

stm(randtS+1,:) = [...
        WcS     WcS     WcS    WcS      WcS    WcS     pgS    RandDelay      0      0  ; ...   % Mandatory time in Cpoke
             ];
stm(pgS+1,:) = [...
        WcS     WcS     WcS    WcS      WcS    WcS     offcliS  value(PreChordTime)      0      4  ; ...   % Mandatory time in Cpoke
             ];
stm(offcliS+1,:) = [...
        offcliS offcliS   offcliS offcliS    offcliS offcliS   sndS  0.01   0     -4  ; ...   % Stop clicks
             ];
...
global fake_rp_box;
if(fake_rp_box == 2)
    % --- Send the state matrix directly to state machine ---
