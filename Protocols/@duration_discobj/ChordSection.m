function [x, y, ...
    sound_len, error_len, tones_list, prechord_list, go_dur, ...
...
switch action,
    case 'init'
        ...
        SoloParamHandle(obj, 'tone1_list', 'value', zeros(1,value(maxtrials)) ); % Potentially variable tone1 durations
           SoloParamHandle(obj, 'effective_dur', 'value', zeros(1,value(maxtrials)) ); % Which duration was presented for this trial?
    ...
    case 'make'
        ...
        % Part 1: The prolonged tone, (say 100 ms)
        % tone_dur is in milliseconds
        % the lists store values in seconds  (tone1_list, tone2_list,
        % tones_list)
        if value(Blocks_Switch) >0
            tone_dur = tones_list(n_done_trials+1)*1000; % 0.1 * 1000 = 100
            if n_done_trials ==0
               tone_dur = 500;
            end;
            if upcoming_side == left
                tone1_list(n_done_trials+1) = tone_dur / 1000; % in seconds               
                tone2_list(n_done_trials+1) = 0;
            else
                tone2_list(n_done_trials+1) = tone_dur / 1000;
                tone1_list(n_done_trials+1) = 0;
            end;
        else % non-psych trials
            if upcoming_side == left
                tone_dur = tone1_list(n_done_trials+1) * 1000;
            else
                tone_dur = tone2_list(n_done_trials+1) * 1000;
            end;
            tones_list(n_done_trials+1) = tone_dur / 1000;
        end;
        effective_dur(n_done_trials+1) = tone_dur;
        
        snd_string = sprintf('%s: (%2.1f KHz for %3.2fms)', sidetxt, value(Tone_Freq), tone_dur);
        ...
        if strcmp(value(Tone_Loc), 'on')
            if stated_side == left % making localized sound - use stated_side
        ...
        % debug print ------------------------------
        %fprintf(1,'\n\n%s:make >>>>>>>>>>\n', mfilename);
        %fprintf(1,'\t%s: (%2.1f KHz for %3.2fms)', sidetxt, value(Tone_Freq), tone_dur*1000);
        %fprintf(1,'<<<<<<<<<<<<<< %s:make \n\n', mfilename);
   ...
   %         fprintf(1,'%i: %i to %i\n', blocknum, sidx, eidx);
       ...
       final_block=sub__block_maker(logmin,logmax, logbins, n2m,sl(eidx+1:end), value(right_is_low));
        ...
        if value(right_is_low) > 0
            temp = value(tones_list);  % 0.450
            oldtemp = temp;
            tempmini = log(temp(n_done_trials+1:end) * 1000);
            tempmini = (log(value(Tone_Dur1)*1000) + log(value(Tone_Dur2)*1000)) - tempmini;
            temp(n_done_trials+1:end) = exp(tempmini) / 1000;
            tones_list.value = temp;
        end;
        
%         % Debug print ---------------------
%         fprintf(1,'\n\n%s:make_blocks: >>>>>>\n', mfilename);
%         fprintf(1,'First ten sides:\n\t');
%         sl(starting_at:starting_at+10)
%         fprintf(1,'\n\n First ten tones\n\t');
%         tmp = value(tones_list);
%         tmp(starting_at:starting_at+10)
%         fprintf(1,'<<<<<<< %s:make_blocks\n\n', mfilename);
