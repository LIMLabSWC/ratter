...
switch action,
    case 'init',
        ...
        DeclareGlobals(obj, 'ro_args', {'Last10num', 'Last15num', ...
            'Last30num', 'Last65num', 'Last100num', 'to_rate_Last15', ...
                            'poke_rate_Last15', 'LeftRewards', 'RightRewards'});
    ...
    case 'update',
        ...
        if isempty(u), % no wat-for-answer poke, must've been direct delivery
            ...
            if isempty(u), 
                offending_rts = rts;
                offending_evs = evs;
                offending_stm = value(state_matrix);
                save('crashed_with_no_wapoke_error.mat', 'offending_rts','offending_evs','offending_stm');
                error(['No wait-for-answer, no dir del state!']); 
            end;
        ...
        % Count rewards
        %        if ~isempty(u1), rew = value(LeftRewards); rew(n_done_trials)=1; LeftRewards.value  = rew;  end;
        %        if ~isempty(u2), rew = value(RightRewards); rew(n_done_trials)=1; RightRewards.value = rew; end;
