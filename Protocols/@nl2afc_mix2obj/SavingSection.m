...
function [x, y] = SavingSection(obj, action, x, y)
   ...
   switch action
    case 'init',      % ------------ CASE INIT --------------------
      ...
      MenuParam(obj, 'ratname', {'xu1_1','xu1_2','xu1_3', 'xu2_3', 'x7','x8','x9','x10'},1, x, y); 
      next_row(y, 1.5);
    ...
    case 'savesets',       % ------------ CASE SAVESETS --------------------
      ...
      if x==1,
         save_solouiparamvalues(value(ratname), 'commit', 0,'interactive', 1);
      else
         save_solouiparamvalues(value(ratname), 'commit', 1,'interactive', 0);
      end;
      return;
    ...
    case 'savedata',       % ------------ CASE SAVEDATA --------------------
      SaveTime.value = datestr(now);      
      save_soloparamvalues(value(ratname), 'commit', 1);
