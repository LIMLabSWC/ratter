...
%         [LeftWValveTime RightWValveTime] = WaterValvesSection(obj, 'get_water_times');
		...
		side = SideSection(obj, 'get_current_side'); %1=left, 0 =right
        light = SideSection(obj, 'get_current_light'); %1=light, 0 = dark
        correction = SideSection(obj, 'get_current_correction'); 
        training = SideSection(obj, 'get_current_training'); 
...
%             SecondHitState = 'secondrighthit';
        ...
        if strcmp(training, 'Pro')
            if light == 0 & correction ==1
                ErrorState  = 'delayedrewarderror'; % going to light error in ProTraining with correction = delayed reward
            else
                ErrorState  = 'norewarderror'; % all other cases = punishment, no reward, time out
            end
        else
            if light == 1 & correction ==1
                ErrorState  = 'delayedrewarderror'; % going to dark error in AntiTraining with correction = delayed reward
            else
                ErrorState  = 'norewarderror'; % all other cases = punishment, no reward, time out
            end
        end
        ...
        if xor(side,light)
            HitEvent = 'Rin';      ErrorEvent = 'Lin'; 
            HitState = 'righthit'; RewardLight  = right1led; 
            SecondHitState = 'secondrighthit';  
        else
            HitEvent = 'Lin';      ErrorEvent = 'Rin'; 
            HitState = 'lefthit';  RewardLight  = left1led; 
            SecondHitState = 'secondlefthit';
        end
        %end
			...
			case 1  %%  center led on -> poke in the center -> go cue -> reward light and sound -- waiting time grows slowlly -stimuli can be present
               ...
               % when SettlingIn_time elapses (settling_period_In) "legal cbreaks" changes to usueal legal_cbreaks 
				...
				sma = add_state(sma, 'name', 'wait_for_collecting_reward', 'self_timer', 30000, ...
					...
					'input_to_statechange',{HitEvent, HitState, ErrorEvent, ErrorState}); 
                
                
                % The no reward Error state:
                %with error sound
                
				sma = add_state(sma,'name', 'norewarderror','self_timer', viol_snd_duration, ...
					'output_actions', {'SoundOut', err_sound_id}, ...					
					'input_to_statechange', {'Tup', 'error_state'});
                
                % The delayed reward Error state:
                %with error sound
                
                sma = add_state(sma,'name', 'delayedrewarderror','self_timer', wait_for_second_hit, ...
					'output_actions', {'DOut', LEDOn*SideLight}, ...					
				...
				sma = add_state(sma,'name', 'lefthit','self_timer', reward_delay, ...
					'output_actions', {'DOut', RewardLight', 'SoundOut', Lreward_sound_id}, ...					
...
%                 
				sma = add_state(sma, 'self_timer', LeftWValveTime, ...
					'output_actions', {'DOut', RewardLight+left1water,},...
				...
				sma = add_state(sma,'name', 'righthit','self_timer', reward_delay, ...
					'output_actions', {'DOut', RewardLight', 'SoundOut', Rreward_sound_id}, ...					
					'input_to_statechange', {'Tup', 'current_state+1'});
...
%                 
				sma = add_state(sma, 'self_timer', RightWValveTime, ...
					'output_actions', {'DOut', RewardLight+right1water,},...
					'input_to_statechange',{'Tup','hit_state'});
                ...
                sma = add_state(sma, 'self_timer', reward_delay, ...
					'output_actions', {'DOut', AnyReward*RewardLight, 'SoundOut', AnyReward*Lreward_sound_id}, ...	
                    'input_to_statechange', {'Tup', 'current_state+1'});
				sma = add_state(sma, 'self_timer', LeftWValveTime, ...
					'output_actions', {'DOut', AnyReward*(RewardLight+left1water)},...
					'input_to_statechange',{'Tup','hit_state'});
				% The two states that make a SecondRightHit:
				sma = add_state(sma,'name', 'secondrighthit','self_timer', secondhit_delay, ...	
                    'input_to_statechange', {'Tup', 'current_state+1'});	
                sma = add_state(sma, 'self_timer', reward_delay, ...
					'output_actions', {'DOut', AnyReward*RewardLight, 'SoundOut', AnyReward*Rreward_sound_id}, ...	
                    'input_to_statechange', {'Tup', 'current_state+1'});
				sma = add_state(sma, 'self_timer', RightWValveTime, ...
					'output_actions', {'DOut', AnyReward*(RewardLight+right1water)},...
					'input_to_statechange',{'Tup','hit_state'});
...
% 			'input_to_statechange',{'Tup','check_next_trial_ready'});
	...
	case 'get_state_colors',
		varargout{1} = struct( ...
            ...
            'secondlefthit',      [0.7   0.7     0.3], ...
            'secondrighthit',       [0.3   0.7     0.7], ...
            'error_state',  [1    0.54 0.54], ...
			'drink_state',           [0    1    0],    ...
            'delayedrewarderror',  [1    0.30 0.54], ...
            'norewarderror',  [1    0.54 0.30], ...
