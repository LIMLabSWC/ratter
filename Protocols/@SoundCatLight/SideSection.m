...
switch action,
	...
	case 'init'
        ...
        ToggleParam(obj, 'correction', 1, x,y,...
			'OnString', 'correction ON',...
			'OffString', 'correction OFF',...
			'TooltipString', sprintf(['If on then it enables the Correction for pro and anti errors']));
		next_row(y);
        ...
        DispParam(obj, 'ThisLight', 'LIGHT', x, y); next_row(y);
		SoloParamHandle(obj, 'previous_sides', 'value', []);
		SoloParamHandle(obj, 'previous_lights', 'value', []);
		DeclareGlobals(obj, 'ro_args', 'previous_sides');
		DeclareGlobals(obj, 'ro_args', 'previous_lights');
        ...
        NumeditParam(obj, 'time_bet_aud1_gocue', 1, x,y,'label','A1-GoCue time','TooltipString','time between the end of the stimulus and the go cue ');
        next_row(y);
	...
	% to modify it for widefield imagine
		...
		next_row(y);
        NumeditParam(obj,'secondhit_delay',2,x,y,'label','SecondHit Delay','TooltipString','Reward will be delayed with this amount if reward_type=DelayedReward');
		...
		next_row(y);		
        ToggleParam(obj, 'ProTraining', 1, x,y,...
			'OnString', 'Using Pro-orienting',...
			'OffString', 'Using Anti-orienting',...
			'TooltipString', sprintf('Pro-only pre-training if ON'));
		next_row(y);		
        ToggleParam(obj, 'ProAntiOnly', 0, x,y,...
			'OnString', 'Pro or Anti Only',...
			'OffString', 'Pro and Anti Trials',...
			'TooltipString', sprintf('Select only Pro or Anti trials based on ProTraining toggle (if ProTraining only Pro trials, if AntiTraining only Anti trials)'));
		next_row(y);
        ...
        SoloFunctionAddVars('StimulusSection', 'ro_args', ...
			{'ThisTrial';'ThisLight';'A1_time';'time_bet_aud1_gocue' ; ...
			'PreStim_time'});
	...
	case 'prepare_next_trial'
		...
		switch value(training_stage)
			...
			case 1,                  %%  center led on -> poke in the center -> go cue -> reward light and sound
                %what A1_time and time before go cue?
                if random_A1_time
                    A1_times = [0.2, 0.4];
		...
		was_hit=false;
        was_timeout=false;
        was_delayedrewarderror=false;
        was_norewarderror=false;
		if n_done_trials>0
			if ~isempty(parsed_events)
				if isfield(parsed_events,'states')
                    ...
                    if isfield(parsed_events.states,'norewarderror')
						was_norewarderror=rows(parsed_events.states.norewarderror)>0
                    end
                    if isfield(parsed_events.states,'delayedrewarderror')
						was_delayedrewarderror=rows(parsed_events.states.delayedrewarderror)>0
					end
				end
            ...
            SideSection(obj,'update_light_history');
			
			if ~was_viol && ~was_timeout
                if ~was_norewarderror && ~was_delayedrewarderror
                    was_hit = true
                end
%                 was_hit=rows(parsed_events.states.second_hit_state)==0;
        ...
        if value(training_stage) ==0 | value(ProAntiOnly)==1
            if value(ProTraining) ==1
                ThisLight.value='LIGHT'; 
            else
                ThisLight.value='DARK'; 
            end
        else
            if (rand(1)<=0.5)
                ThisLight.value='LIGHT';
...
%         right_wtr_mult.value=maxasymp + (minasymp./(1+(n_done_trials/inflp).^slp).^assym);
    ...
    case 'get_previous_lights',
		x = value(previous_lights); %#ok<NODEF>
	...
	case 'update_light_history'
		if strcmp(ThisLight, 'LIGHT')
			ps=value(previous_lights);
			ps(n_done_trials)=1;
			previous_lights.value=ps;
