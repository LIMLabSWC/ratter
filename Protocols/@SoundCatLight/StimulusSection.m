...
switch action,
  ...
  case 'init'
    ...
    next_row(y);
    next_row(y);
    MenuParam(obj, 'Rule', {'S2>S_boundry Light','S2>S_boundry Dark'}, ...
      'S2>S_boundry Light', x, y, 'labelfraction', 0.35, 'TooltipString', sprintf(['\nThis bottom determines the rule\n', ...
      '\n''S2>S_boundry Light'' means if Aud2 > Aud_boundry then reward will be delivered from the illuminated water spout and if Aud2 < Aud_boundry then water comes form the other\n',...
      '\n''S2>S_boundry Dark'' means if Aud2 < Aud_boundry then reward will be delivered from the illuminated water spout and if Aud2 > Aud_boundry then water comes from other\n'])); 
    next_row(y, 1)
...
%     next_row(y);
    ...
    next_row(y);
	NumeditParam(obj,'minF1',7,x,y,'label','minF1','TooltipString','min frequency value for AUD1');
    next_row(y);
	NumeditParam(obj,'maxF1',16,x,y,'label','maxF1','TooltipString','max frequency value for AUD1');
    next_row(y);
    ...
    next_row(y);
 	...
 	next_row(y);
    NumeditParam(obj,'nCatch',1,x,y,'label','Num Catch S1','TooltipString','Number of catch stimuli');
    next_row(y);
    NumeditParam(obj,'probcatch',0.05,x,y,'label','probcatch','TooltipString','probability of having a catch stimulus as the stimulus at each trial');
    next_row(y);   
    MenuParam(obj, 'DistributionType', {'narrow_gauss','wide_gauss','bimodal', 'gmm', 'uniform', 'unim-unif'}, ...
    ...
    boundary.value = median(1:value(numClass));
    mu.value = median(1:value(numClass));
  ...
  case 'prepare_next_trial' 
    ...
    %disp(value(A1_sigma))
    %disp(value(A1_freq))
    if thisclass(n_done_trials+1)> value(boundary)%value(numClass)
        set(value(h1), 'YData', log(value(A1)), 'color',[0.4 0.8 0.1],'markerfacecolor',[0.4 0.8 0.1]);
    else
        set(value(h1), 'YData', log(value(A1)), 'color',[0.8 0.4 0.1],'markerfacecolor',[0.8 0.4 0.1]);
    end

    %% which A1_time?
    
%     A1_times = [0.4, 0.6, 0.8];
%     randomix = randi(length(A1_times), 1);
%     A1_time.value = value(A1_times(randomix));
%     value(A1_time)
%    A1_time = 0.2;
...
%       load_solouiparamvalues(obj, 'ratname', 'rescan_during_load', varhandles2);
    ...
    if strcmp(DistributionType,'uniform')
        ...
        mu = value(mu);%median(1:value(numClass));
...
%             end
    ...
    if catch_trials == 1
        ...
        if nCatch >1
            ...
            catch_bag = ones(1,max_trials_catch)*(value(numClass)+1);
        end
...
%         end 
    ...
    if frequency_categorization
     ...
     if strcmp(Rule,'S2>S_boundry Light')
        if strcmp(ThisLight, 'LIGHT')
%             bag=[];
...
%             end
            ...
            ix_high = ix(ix>=value(boundary));%ix(ix>=median(1:value(numClass)));
...
%             end
            ...
            ix_low = ix(ix<=value(boundary));%ix(ix<=median(1:value(numClass)));
    ...
    elseif strcmp(Rule,'S2>S_boundry Dark')
        if strcmp(ThisLight, 'DARK')
%             bag=[];
%             for cc=1:value(numClass)/2
%                 eval(sprintf('pr=value(probClass%d);',cc+value(numClass)/2)); 
%                 bag=[bag ones(1,(10)*pr)*(cc+value(numClass)/2)];
...
%             end
    ...
    %% Case make_stimuli
    case 'make_stimuli'
   %% make numClass of sigma or frequency values 
    S1.value = [];
    ...
    F1.value = [];
    F1(1)=log(value(minF1));
    F1(value(numClass))=log(value(maxF1));
    steps = (F1(value(numClass)) - F1(1))./(value(numClass)-1);
    for ii=2:value(numClass)-1
        F1(ii)=F1(ii-1)+steps;    
    end   
...
%      Xtick=get(value(ax),'XtickLabel');
    ...
    %% plot the stimulus;
    if value(thisclass) > value(boundary)%value(numClass)
        h1.value=plot(xd,log(value(A1)),'s','color',[0.4 0.8 0.1],'markerfacecolor',[0.4 0.8 0.1],'MarkerSize',15,'LineWidth',3);
    else
        h1.value=plot(xd,log(value(A1)),'s','color',[0.8 0.4 0.1],'markerfacecolor',[0.8 0.4 0.1],'MarkerSize',15,'LineWidth',3);
    end
  ...
  %% Case Plot_perf
    ...
    case 'CatchTrials'
        if catch_trials == 1
            ...
            if nCatch ==1
                S1_catch.value = [];
                S1_catch(1) = median(value(S1));
            else
%             for 3 catch classes around and including boundary     
                S1_catch.value = [];
