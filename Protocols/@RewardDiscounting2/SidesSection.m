...
% -----------------------------------------------------------------------
  ...
  case 'update_plot',
    ...
    nr = value(nonreward_history);
    sh = sh(1:length(hh));
    ch = ch(1:length(hh));
    
    if n_done_trials >= 1  
        reds   = find(hh==0 & sh~='f');
        greens = find(hh==1 & sh=='r');
        blues  = find(hh==1 & sh=='l');
        magens = find(nr~='.');
    ...
    plot(ax, greens, ones(1,length(greens))*2, 'g.');
    hold(ax, 'on');
    plot(ax, blues,  ones(1,length(blues)),    'b.');
    plot(ax, reds,   (sh(reds)  =='r')+1,      'r.'); 
    plot(ax, magens, (sh(magens)=='r')+1,      'm.');
