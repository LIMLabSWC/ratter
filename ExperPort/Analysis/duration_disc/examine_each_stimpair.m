function [] = examine_each_stimpair(rat, task, idxlist, varargin)
  ...
  figure;
  set(double(gcf),'Menubar','none','Toolbar','none', 'Position', [200 200 900 ...
                      600]);
  set(double(gcf),'Name', ...
  ...
  for k = 1:min(4, length(idxlist))
    ...
    %  legend({'Left', '', 'Right'}, 'Location', 'NorthWest'); legend('boxoff');
      set(double(gca), 'Xlim', [ 0 length(leftrates)+1 ], ...
               'YLim', [min(min(leftrates), min(rightrates))-0.1 1.1]);
      title(s);
    end;
  end;
  
  uicontrol(double(gcf), 'Style','text','Position', [10 20 250 15], ...
            'String', 'Not all indices may be visited in every session', ...
            'FontAngle', 'italic', 'Background', 'white');
  uicontrol(double(gcf), 'Style', 'text', 'Position', [10 10 250 15], ...
  ...
  for k = 1:min(4, length(idxlist))
    ...
    set(double(gca), 'XLim', [ 0 length(idx_data{k,4})+1 ], ...
             'YLim', [min(idx_data{k,4})-10 max(idx_data{k,4})+10]);
    xl = get(double(gca),'XLim');  yl = get(double(gca),'YLim');
