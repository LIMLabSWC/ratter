...
set(double(gca),'XLim',[1-0.2 2+0.2], 'XTick',[1 2], 'XTickLabel', grp,'YLim', ylim);
set(double(gcf),'Position',[ 213   421   266   471]);
title(area_filter);
axes__format(double(gca));

% STEP 2: now get residuals
[resid fnames_resid grp_resid] = surgery_effect_residuals('area_filter', 'ACx');
set(double(gcf),'Position',[860   421   266   471]);
...
set(double(gca),'XLim', [x(1)-0.02 x(2)+0.02]);
set(double(gcf),'Position',[204    78   529   268],'Toolbar','none','Menubar','figure');
...
set(double(gca),'XLim',[0 length(mega_hdiff)+1], 'XTick', 1:length(mega_hdiff), ...
    'XTickLabel', mega_fnames);
title('Impairment levels of individual rats');
axes__format(double(gca));
ylabel('% change in performance');
set(double(gcf),'Position',  [385 628 1018 243]);
set(double(gca),'Position',[0.08 0.1 0.88 0.75])
