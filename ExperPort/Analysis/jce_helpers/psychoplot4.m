...
tt=prctile(x_vals,[5 95]);
bin_e=linspace(tt(1),tt(2),15);

[n,ni]=histc(x_vals,bin_e);
for nx=1:(numel(bin_e)-1)
   muB(nx)=nanmean(went_right(ni==nx));
   seB(nx)= sqrt(muB(nx)*(1-muB(nx))/sum(ni == nx));
% 		sqrt(sum(ni==nx));   
end
binc=bin_e(1:end-1)+diff(bin_e(4:5))/2;
[h]=errorplot(binc,muB,seB);
