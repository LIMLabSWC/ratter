...
extern mclComponentData __MCC_GCS_receiver_component_data;
 ...
 */
#ifndef LIB_GCS_receiver_C_API 
#define LIB_GCS_receiver_C_API /* No special import/export declaration */
#endif

LIB_GCS_receiver_C_API 
bool MW_CALL_CONV GCS_receiverInitializeWithHandlers(
...
)
{
  ...
  if (!mclInitializeComponentInstanceWithEmbeddedCTF(&_mcr_inst,
                                                     &__MCC_GCS_receiver_component_data,
                                                     true, NoObjectType,
                                                     ExeTarget, error_handler,
                                                     print_handler, 2285941, NULL))
    return false;
  return true;
}

LIB_GCS_receiver_C_API 
bool MW_CALL_CONV GCS_receiverInitialize(void)
{
  return GCS_receiverInitializeWithHandlers(mclDefaultErrorHandler,
                                            mclDefaultPrintHandler);
}

LIB_GCS_receiver_C_API 
void MW_CALL_CONV GCS_receiverTerminate(void)
{
  if (_mcr_inst != NULL)
...
int run_main(int argc, const char **argv)
{
  ...
  __MCC_GCS_receiver_component_data.path_to_component = path_to_component; 
  if (!GCS_receiverInitialize()) {
    return -1;
  }
  argc = mclSetCmdLineUserData(mclGetID(_mcr_inst), argc, argv);
  _retval = mclMain(_mcr_inst, argc, argv, "GCS_receiver", 0);
  if (_retval == 0 /* no error */) mclWaitForFiguresToDie(NULL);
  GCS_receiverTerminate();
...
int main(int argc, const char **argv)
{
  if (!mclInitializeApplication(
    __MCC_GCS_receiver_component_data.runtime_options,
    __MCC_GCS_receiver_component_data.runtime_option_count))
    return 0;
