...
% ======================
% This script initializes the BControl system, setting up paths, loading 
% settings, and preparing the environment for behavioral experiments.
%
% Usage:
...
if isempty(BpodSystem)
    ...
    error(['Error: You must run Bpod() at the command line, ' ...
           'before you can open B-control']);
end
...
dbstop if error;
dbstop if caught error 'TIMERERROR:catcherror';  % For debugging timer loop
...
[Protocols_Directory errID errmsg] = bSettings('get', 'GENERAL', ...
    'Protocols_Directory');
fprintf('Protocols_Directory from settings: %s\n', Protocols_Directory);
fprintf('Current directory: %s\n', pwd);
% Only construct full path if Protocols_Directory is relative
if ~isempty(Protocols_Directory) && ...
        ~strcmp(Protocols_Directory(1), filesep) && ...
        ~strcmp(Protocols_Directory(2), ':')
...
if ~exist(FILENAME__CUSTOM_SETTINGS,'file'),
    [errID errmsg] = BControl_First_Run(FILENAME__CUSTOM_SETTINGS, ...
...
elseif strcmpi(dbstop_setting,'never') || strcmpi(dbstop_setting,'NULL') || ... 
...
function [errID, errmsg] = BControl_First_Run(FILENAME__CUSTOM_SETTINGS, ...
    FILENAME__SETTINGS_TEMPLATE)
...
if ~exist(FILENAME__CUSTOM_SETTINGS,'file'),
    if ~exist(FILENAME__SETTINGS_TEMPLATE,'file'),
        errID = 1;
        errmsg = [errorlocation ': Could not find settings template file' ...
            ' at "' FILENAME__SETTINGS_TEMPLATE '".'];
        return;
    else
        [copysuccess,copymessage] = copyfile(FILENAME__SETTINGS_TEMPLATE, ...
            FILENAME__CUSTOM_SETTINGS, 'f');
        if ~copysuccess,
            errID = 1;
            errmsg = [errorlocation ': Copying template settings file to' ...
                ' custom settings file failed. Copy error message: ' ...
                copymessage];
            return;
        else
...
[Main_Code_Directory errID errmsg] = bSettings('get', 'GENERAL', ...
    'Main_Code_Directory');
if errID, return; end;
...
[Main_Data_Directory errID errmsg] = bSettings('get', 'GENERAL', ...
    'Main_Data_Directory');
if errID, return; end;
...
if ~ischar(steMS) || ~ischar(sndMS) ...
        || (ismember(fake_rp_box, [0 1 2 20]) && ...
            (isempty(steMS) || isempty(sndMS))),
...
[state_machine_server errID errmsg] = bSettings('get', 'RIGS', ...
    'state_machine_server');
if errID, return; end;

global sound_machine_server;
[sound_machine_server errID errmsg] = bSettings('get', 'RIGS', ...
    'sound_machine_server');
if errID, return; end;
...
if errID_i,
    errID = 1;
    errmsg = [errorlocation ': Attempt to retrieve DIOLINES settings group' ...
        ' failed. bSettings(''get'',''DIOLINES'',''all'') returned the' ...
        ' following error (ID: ' int2str(errID_i) '): ' errmsg_i ];
    return;
end;
