...
switch action
 case 'init',
   ...
   NumeditParam(obj,'StdITI', 1, x, y, 'position', [x y 100 20], 'TooltipString', ...
                ['Standard dev. of the inter-trial interval ' ...
                 'distribution']); 
   NumeditParam(obj, 'MeanITI', 3, x, y,'position', [x+100 y 100 20],  ...
                'TooltipString', ['Mean of the distribution of ' ...
                       'inter-trial intervals']); next_row(y);   
   DispParam(obj, 'CurrentITI', 0, x, y); next_row(y);
   
   NumeditParam(obj, 'MeanValveDelay', 0.2, x, y, 'TooltipString', ['Mean of the distribution of'...
       'odor valve delay']); next_row(y);
   
   DispParam(obj, 'rand_valve_delay', 0, x, y, 'TooltipString',...
       'an exponentially random delay between nose poke and valve open');
   next_row(y);
   
   NumeditParam(obj, 'valid_samp_time', 0.2, x, y, 'TooltipString',...
   ...
   SoloFunctionAddVars('make_and_upload_state_matrix', 'ro_args', {'valid_samp_time', 'rand_valve_delay', 'max_odor_time'});
 ...
 case 'comput_intervals'
   ...
   temp1(n_done_trials+1:end) =  exprnd(value(MeanValveDelay),[len1 1]);
