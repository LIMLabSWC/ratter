...
%%%  $Revision: 1974 $
%%%  $Date: 2009-03-15 15:46:00 -0400 (Sun, 15 Mar 2009) $
%%%  $Source$
...
switch action
  case 'init',
    ...
    %ProbeFrequencies.value = logspace(log10(5010),log10(40200),17)';
    ...
    SoloParamHandle(obj, 'FrequencyOrder','value', ...
                    nan(value(MaxTrials),value(NtonesPerTrial)));    
    SoloParamHandle(obj, 'FrequencyOrderExtraTone','value', ...
                    nan(value(MaxTrials),value(NtonesPerTrial)));    
    ...
    EachDistractorSource.values = nan(value(MaxTrials),value(NtonesPerTrial));
  ...
  case 'update_calibration'
    ...
    switch(value(Calibration))
      case 'speakersstereo'
        SpeakerCalibrationFile = fullfile(SettingsDir,'SpeakerStereoCalibration.mat');        
      case 'speakers'
      ...
      case 'speakersBIASED'
        SpeakerCalibrationFile = fullfile(SettingsDir,'SpeakerCalibrationBIASED.mat');
      otherwise
    ...
    if(exist(SpeakerCalibrationFile,'file'));
        ...
        if(size(SpeakerCalibration.AttenuationVector,2)==2)
            SpeakerCalibration.AttenuationVector = mean(SpeakerCalibration.AttenuationVector,2);
        end
  ...
  case 'update_sound_this_trial'
    ...
    % -- Add extra tone to target if required --
    if(strcmp(value(ExtraTone),'on'))
        ThisTrialOtherFrequency = TargetFrequencies(3-NextRewardSide)*FreqFactor;
        GenericSound.Frequency= ThisTrialOtherFrequency;    % Hz
        GenericSound.ModIndex = 0;       % Make this one a tone
        GenericSound.Waveform = [];
        GenericSound.Attenuation = SoundsSection(obj,'calculate_attenuation',SpeakerCalibration,...
                                             GenericSound.Frequency,GenericSound.Volume);
        ExtraToneForTargetWaveform = SoundsSection(obj,'create_waveform',GenericSound);
        TargetWaveform = TargetWaveform+ExtraToneForTargetWaveform;
    end
    ...
    if strcmp(value(DistractorSource),'monaural-random');
         EachDistractorSource.values(n_done_trials+1,:) = ceil(2*rand(size(TonesOrder)));
    else
         EachDistractorSource.values(n_done_trials+1,:) = zeros(size(TonesOrder));
    end
    
    % -- Create distractors --
    FrequencyOrder(n_done_trials+1,:) = TonesOrder;
    ...
    GenericSound.TonesSource  = EachDistractorSource.values(n_done_trials+1,:);
    ...
    % -- Add extra tones to distractors if required --
    if(strcmp(value(ExtraTone),'on'))
        %ExtraTonesOrder = ceil(length(value(ProbeFrequencies))*rand(1,value(NtonesPerTrial)));
        ExtraTonesOrder = companion_tones(TonesOrder, value(ProbeFrequencies));
        GenericSound.TonesFrequency= ProbeFrequencies(ExtraTonesOrder)*FreqFactor;    % Hz
        GenericSound.Waveform = [];
        GenericSound.Attenuation = SoundsSection(obj,'calculate_attenuation',SpeakerCalibration,...
                                             GenericSound.TonesFrequency,GenericSound.Volume);
        ExtraTonesWaveform = SoundsSection(obj,'create_waveform',GenericSound);
        DistractorsWaveform = DistractorsWaveform+ExtraTonesWaveform;

        FrequencyOrderExtraTone(n_done_trials+1,:) = ExtraTonesOrder;
  ...
  case 'create_waveform'
    ...
    switch ThisSound.Type
      case 'Tone'
        ...
        ThisSound.Waveform = apply_risefall(ThisSound.Waveform,RaiseFallDuration,srate);
      case 'Noise'
      ...
      case 'FMchord'
        ...
        ThisSound.Waveform = apply_risefall(mean(ThisSound.Waveform,2),RaiseFallDuration,srate);
      case 'OLD_TonesTrain'
        ...
        for indtone=1:length(ThisSound.TonesFrequency)
            ...
            ThisTone = apply_risefall(ThisTone,RaiseFallDuration,srate);
      ...
      case 'ChordsTrain'
        ...
        for indchord=1:length(ThisSound.TonesFrequency)
            ...
            ThisChord = apply_risefall(mean(ThisChord,2),RaiseFallDuration,srate);
...
% -------------------- FUNCTION -------------------------
function ExtraTonesIndexes = companion_tones(TonesIndexes, ProbeFreqs)
    %%% So far designed for odd number of possible frequencies %%%
    NpossibleFreqs = length(ProbeFreqs);
    MidIndex = round(NpossibleFreqs/2);
    PossibleIndLowerHalf = [1:MidIndex-1];
    PossibleIndHigherHalf = [MidIndex+1:NpossibleFreqs];
    TonesLowerHalf = TonesIndexes<MidIndex;
    TonesHigherHalf = TonesIndexes>MidIndex;
    TonesMidPoint = TonesIndexes==MidIndex;
    ExtraTonesIndexes = zeros(size(TonesIndexes));
    ExtraTonesIndexes(TonesHigherHalf) = PossibleIndLowerHalf(ceil(length(PossibleIndLowerHalf)*rand(1,sum(TonesHigherHalf))));
    ExtraTonesIndexes(TonesLowerHalf) = PossibleIndHigherHalf(ceil(length(PossibleIndHigherHalf)*rand(1,sum(TonesLowerHalf))));
    ExtraTonesIndexes(TonesMidPoint) = ceil(NpossibleFreqs*rand(1,sum(TonesMidPoint)));
return
% ----------------- END OF FUNCTION ----------------------
