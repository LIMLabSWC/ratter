...
%%%  $Revision: 675 $
%%%  $Date: 2007-09-26 12:08:34 -0400 (Wed, 26 Sep 2007) $
%%%  $Source$


function [x, y, WaterDeliverySPH, RelevantSideSPH,CueEveryNtrialsSPH] = SidesSection(obj, action, varargin)
...
switch action
  case 'init',
    ...
    next_row(y, 0.5);
    NumeditParam(obj, 'CueEveryNtrialsSPH', 10, x, y,...
                 'label','CueEveryNtrials'); next_row(y);
    %%%set_callback(CueEveryNtrials, {mfilename, 'update_distractorlist'});
    %%%set(get_ghandle(CueEveryNtrials),'Enable','off');
   
    NumeditParam(obj, 'IncongruentProb', 0.8, x, y,...
  ...
  case 'update_distractorlist'
    FutureIndexes = (n_done_trials+1:value(MaxTrials));
    % -- Set future irrelevant stim (1:left 0:right) --
    ...
    if(n_done_trials==0)
        ...
        SidesPlotSection(obj, 'update', n_done_trials, value(RewardSideList),[],IncongruentTrials);        
    end
