...
%        '\nStimTimeStart to FromAnswerPoke. No loop sound ==> sound plays only for specified num of cycles']));
      ...
      DispParam(obj, 'CurrentPair', 1, x, y, ...
          'position', [x y 100 20], ...
          'labelfraction', 0.7, ...
          'TooltipString', 'Current stimulus pair used, corresponds to table in Sounds Panel');
      ...
      % valid.  The status of the system is displayed as a message in the
      % main protocol window.  The flag and the message are set by action
      % 'check_norm'
      SoloParamHandle(obj, 'go_flg', 'value', 0);  
      SubheaderParam(obj, 'message', '', x, y);
      set(get_ghandle(message), 'BackgroundColor', 'r'); next_row(y);
