function [obj] = synced_durationobj(a)
...
if nargin==1 && ischar(a), % <~> isstr to ischar 2007.09.24
    SoloParamHandle(obj, 'protocol_name', 'value', lower(a));
end;

fig_position = [398    76   662   695];
...
NumeditParam(obj,'sync_dur', 0.3, x, y, 'label', 'Sync Dur', 'TooltipString', 'Duration (in milliseconds) of each of the 18 sync states','labelfraction', 0.5);
set(get_lhandle(sync_dur),'BackgroundColor',[1 0.5 0]);
...
ro_args = {'n_done_trials', 'n_started_trials', 'hit_history','maxtrials','Blocks_Switch','Num2Make','Num_Bins'};
...
ro_args = {'side_list', 'n_started_trials', 'n_done_trials', ...
    'maxtrials', 'vpd_list','Num_Bins','Num2Make','Blocks_Switch'};
...
SoloFunction('make_and_upload_state_matrix', ...
    'rw_args', {'RealTimeStates', 'super', 'stm_ctr','protocol_start_time'}, ...
    'ro_args', {'n_done_trials', 'sync_dur', ...
    'side_list', 'vpd_list', ...
    'chord_sound_len', 'error_sound_len', 'tone1_list', 'tone2_list', 'prechord_list', 'go_dur', ...
    ...
    'LegalSkipOut', 'Granularity', 'Max_Trials','MaxMins', 'volume_factor', ...
    });
...
SoloFunctionAddVars('SessionDefinition', 'ro_args', {'myfig','ratname','experimenter'}); % <~> SoloFunction -> SoloFunctionAddVars
...
% HeaderParam(obj, 'prot_title', 'Duration Discrimination', ...
%     x, y, 'width', fig_position(3));
%g = get_ghandle(prot_title); set(g,'BackgroundColor', [138/255 0 77/255],'ForegroundColor',[1 1 1],'Position',[0 0.95 0.05 1]);
