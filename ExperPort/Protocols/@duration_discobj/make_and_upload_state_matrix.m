...
switch action,
    ...
    case 'init',
        ...
        %   SoloParamHandle(obj, 'state_matrix_cell', 'value', {});
        ...
        SoloParamHandle(obj, 'iti_badboy_len', 'value', bb_len);
...
if n_done_trials >= Max_Trials || etime(clock, value(protocol_start_time))/60 > MaxMins,
    ...
    srate = get_generic('sampling_rate');
    white_noise_len   = 2;
    white_noise_sound = 0.1*randn(1,floor(white_noise_len*srate));
    whln = white_noise_len;

    rpbox('loadrp3stereosound__withid', 0.3*value(white_noise_sound), whsd);    
...
stm = [stm ; ...
    ...
    2+LddS 2+LddS  2+LddS  2+LddS  2+LddS 2+LddS  LgcS   drkt+2      0       0 ; ... %5 : Wait for L water collection
    ...
    2+RddS 2+RddS 2+RddS 2+RddS  2+RddS    2+RddS  RgcS   drkt+2      0       0 ; ... %7 : Wait for R water collection
    ];
% end;
...
[stm, ITI_endstate] = ...
    ...
    'state_sound_len', wnln, ...
    'state_sound', whsd);
[stm, ITIReinit_endstate] = sub__noise_section(stm, ...
    ...
    'state_sound_len', wnln, ...
    'state_sound', whsd);
[stm, ExtraITI_endstate] = sub__noise_section(stm, ...
...
if ~exist('state_matrix', 'var'),
    ...
    DeclareGlobals(obj, 'ro_args', {'state_matrix'});
end;
state_matrix.value = stm;
