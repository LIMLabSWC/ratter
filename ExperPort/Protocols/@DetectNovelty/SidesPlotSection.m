...
switch action,
  case 'init',      % ------------ CASE INIT --------------------
    ...
    SoloParamHandle(obj, 'hAxesSides', 'saveable', 0, 'value', axes('Position', [0.1, 0.65, 0.8, 0.28])); % axes
    ...
    set(value(hAxesSides), 'YTick', [-2 -1 0 1 2], 'YTickLabel', {'Novel = 3rd' 'Novel = 4th', 'Novel = 5th', 'Novel = 6th','Novel = 7th'});
  ...
  case 'update',     % ------------ CASE UPDATE --------------------
    ...
    SideListNumeric1 = SideList - 3;
    %SideListNumeric = 1 - SideListNumeric1;
    SideListNumeric = SideListNumeric1;
  ...
  case 'update_xlim'
    ...
    Matches =  ceil(3.*rand(MaxTrials,1)); 
    Mismatches = ceil(3.*rand(MaxTrials,1)); 
