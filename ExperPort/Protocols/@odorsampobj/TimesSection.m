function [x, y] = TimesSection(obj, action, x, y)
   ...
   switch action
    case 'init',
      ...
      NumeditParam(obj, 'MaxOdorTime', 3, x, y);
      next_row(y);
      
      NumeditParam(obj, 'WatAvailTime', 8, x, y);
      next_row(y);
      ...
      SoloFunctionAddVars('make_and_upload_state_matrix', 'ro_args', ...
                          {'ExtraITIOnError', 'DrinkTime', 'ValidPokeTime', 'MaxOdorTime', 'WatAvailTime'});
      ...
      MenuParam(obj, 'task_schedule', {'schedule1' 'schedule2' 'schedule3' 'schedule4' 'schedule5'},1,x,y);
     ...
     case 'update_plot'     
       ...
       if value(ValidPokeTime) >= 0.2
           WatAvailTime.value = 6; % water available within 6 sec after center out
       end
       if value(ValidPokeTime) >= 0.4
           WatAvailTime.value = 4;
       end
       UserPush.value = 0;
   ...
   case 'set_schedules'
       switch value(task_schedule)
           ...
           case 'schedule3'
               delete_sphandle('name','p_xdata'); delete_sphandle('name', 'p_ydata');
               SoloParamHandle(obj, 'p_xdata', 'value', [1:value(total_correct_trials)]);
               SoloParamHandle(obj, 'p_ydata', 'value', [0.250*ones(1,value(total_correct_trials))]);
               set(value(p), 'XData', value(p_xdata));
               set(value(p), 'YData', value(p_ydata));
               set(value(h),'XLim',[0 value(total_correct_trials)],'YLim',[0 0.5]);
               pos = get(get_ghandle(ValidPokeTime), 'position');
           ...
           case 'schedule5'
               ...
               min_poke_time = 0.01;
               max_poke_time = 0.4;
               rand_vals = min_poke_time + ((max_poke_time - min_poke_time) .* rand(1, value(total_correct_trials)));
               SoloParamHandle(obj, 'p_ydata', 'value', rand_vals);
