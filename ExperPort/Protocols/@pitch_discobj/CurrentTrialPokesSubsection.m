...
switch action,
    case 'init',  % ---------- CASE INIT ----------
        ...
        % [WaitForCPoke, WaitForSPoke, RewardStart, ItiStart] = statenames(obj);
        ...
        SoloParamHandle(obj, 'histhandles', ...
            'value', hist_cpoke_stats_SC(cpokestats,'SC', SC, 'plottables', plottables), ...
            'saveable', 0);

        % An axis for the color codes and labels:
        SoloParamHandle(obj, 'axhistlabels', 'saveable', 0, ...
            'value', axes('Position', [0.15 0.2 0.7 0.015]));
        hist_cpoke_stats_SC('make_labels', 'SC', SC, 'plottables', plottables);
        ...
        if strcmp(value(CurrentTrialPokes), 'view'),
            if n_started_trials >= 1,
                ...
                if iscell(RHistory) && all(size(LHistory)==size(RHistory)),
                    ...
                    hist_cpoke_stats_SC(cpokestats, 'handles', value(histhandles), 'SC', SC, 'plottables', plottables);
                end;
                drawnow;
            end;
        end;
