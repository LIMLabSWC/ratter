...
switch action,
    case 'init',  % ---------- CASE INIT ----------
        ...
        set(value(myfig),'Position',[1 screen_size(4)-700, 400 540]); % put fig
        % at top left
        set(value(myfig), ...
    ...
    case 'redraw', % ------- CASE REDRAW
        ...
        if strcmp(value(CurrentTrialPokes), 'view'),
            if n_started_trials >= 1,
                ...
                LHistory = LastTrialEvents_history;

                if length(LHistory) < length(RHistory)
    ...
    case 'hide_cpoke_hist'
        if strcmpi(value(hide_hist), 'hide')
            ...
            kids = get(value(axhist), 'Children');
            for k = 1:length(kids), set(kids(k), 'Visible','off');end;

            kids = get(value(axhistlabels), 'Children');
            for k = 1:length(kids), set(kids(k), 'Visible','off');end;
            pos_pt = get(value(axpatches), 'Position');
            ht = min(pos_pt(4)+value(room), 0.79);
            lower_y = max(pos_pt(2)-value(room), 0.15);
            set(value(axpatches), 'Position', [pos_pt(1) lower_y pos_pt(3) ht]);
        else
            set(value(axhist), 'Visible', 'on');
            kids = get(value(axhist), 'Children');
            for k = 1:length(kids), set(kids(k), 'Visible','on');end;

            kids = get(value(axhistlabels), 'Children');
            for k = 1:length(kids), set(kids(k), 'Visible','on');end;

            pos_pt = get(value(axpatches), 'Position');
            ht = max(pos_pt(4)-value(room), 0.59);
            set(value(axpatches), 'Position', [pos_pt(1) pos_pt(2)+value(room) pos_pt(3) ht]);
        end;
