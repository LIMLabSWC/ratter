function [x, y] = OdorSection(obj, action, x, y)
   ...
   switch action
    case 'init',   % ---------- CASE INIT -------------
      ...
      NumeditParam(obj, 'vr3_4', 0, x, y, 'position', [x+120, y, 80, 20],'label','R_vid','labelpos','left','labelfraction',0.5);
      NumeditParam(obj, 'vr3_3', 0, x, y, 'position', [x+80, y, 80, 20],'label','R_vid','labelpos','left','labelfraction',0.5);
      NumeditParam(obj, 'vr3_2', 0, x, y, 'position', [x+40, y, 80, 20],'label','R_vid','labelpos','left','labelfraction',0.5);
      NumeditParam(obj, 'vr3_1', 6, x, y, 'position', [x, y, 80, 20],'label','R_vid','labelpos','left','labelfraction',0.5);
      next_row(y);
      NumeditParam(obj, 'vl3_4', 0, x, y, 'position', [x+120, y, 80, 20],'label','L_vid','labelpos','left','labelfraction',0.5);
      NumeditParam(obj, 'vl3_3', 0, x, y, 'position', [x+80, y, 80, 20],'label','L_vid','labelpos','left','labelfraction',0.5);
      NumeditParam(obj, 'vl3_2', 0, x, y, 'position', [x+40, y, 80, 20],'label','L_vid','labelpos','left','labelfraction',0.5);
      NumeditParam(obj, 'vl3_1', 5, x, y, 'position', [x, y, 80, 20],'label','L_vid','labelpos','left','labelfraction',0.5);
      next_row(y);
      NumeditParam(obj, 'mix3', 8, x, y, 'position', [x y 100 20], 'label','Mix3','labelpos','left');
      ...
      all_mix.value = [value(mix1); value(mix2); value(mix3)]; %  value(mix4) value(mix5); value(mix6); value(mix7)];
 ...
 %      for i = 2:length(OdorID_bg), bgids=[bgids ; i]; end;
  %    odorset(1,2) = {'Pure'}; % redifine the first odor in the odorset list as pure target odor.
     % bgnames = odorset(OdorID_bg,2);
      set_callback({mix1, mix2, mix3, ...
          ...mix4,mix5, mix6, mix7,...
          probe1,probe2,probe3,...
          ...probe4,probe5,probe6,probe7,...
          vl1_1,vl1_2,vl1_3,vl1_4,vr1_1,vr1_2,vr1_3,vr1_4, vl2_1,vl2_2,vl2_3,vl2_4,...
          vr2_1,vr2_2,vr2_3,vr2_4, vl3_1,vl3_2,vl3_3,vl3_4, vr3_1,vr3_2,vr3_3,vr3_4}, {mfilename, 'write_olf'});
      
      MenuParam(obj, 'mix_ID', {1,2,3,4,5,6,7}, 1, x, y, 'position', [x y 150 20],...
          'label','Current Difference','labelpos', 'left', 'labelfraction',0.7);
      % SoloParamHandle(obj,'Bgrd_Names','value',bgnames);
      DispParam(obj, 'mix_diff', all_mix(1), x, y,'position', [x+150 y 50 20],'label','','labelfraction',0.05);
      SoloFunctionAddVars('BlockControl', 'rw_args', {'all_mix','mix_diff','mix_ID','L_valve','R_valve'});
   ...
   case 'write_olf'
       ...
       valve3_l = [value(vl3_1) value(vl3_2) value(vl3_3) value(vl3_4)]; valve3_l(valve3_l(:)==0)=[];
       valve3_r = [value(vr3_1) value(vr3_2) value(vr3_3) value(vr3_4)]; valve3_r(valve3_r(:)==0)=[];
       
       valve_sets.value = {{valve1_l, valve1_r}, {valve2_l, valve2_r}, {valve3_l, valve3_r}};
       probe_mix.value = [];
