...
switch action
 case 'init',
   ...
   NumeditParam(obj,'StdITI', 0.02, x, y, 'position', [x y 100 20], 'TooltipString', ...
                ['Standard dev. of the inter-trial interval ' ...
                 'distribution']); 
   NumeditParam(obj, 'MeanITI', 0.1, x, y,'position', [x+100 y 100 20],  ...
                'TooltipString', ['Mean of the distribution of ' ...
                       'inter-trial intervals']); next_row(y);   
   DispParam(obj, 'CurrentITI', 0, x, y); next_row(y);
   
   
   NumeditParam(obj, 'MeanRandmDelay', 0.1, x, y, 'TooltipString', ['Mean of the distribution of'...
   ...
   next_row(y);
   NumeditParam(obj, 'odor_travel_time', 0.3, x, y, 'TooltipString',...
       'time after odor bank on but before final valve on'); % allow odor travel from filter to final valve.
   next_row(y);
   NumeditParam(obj, 'valid_samp_time', 0.1, x, y, 'TooltipString',...
       'time that the rat has to stay in the center for valide odor sampling');
   next_row(y);
   
   SoloFunctionAddVars('make_and_upload_state_matrix', 'ro_args', {'valid_samp_time', 'rand_valve_delay','odor_travel_time'});
