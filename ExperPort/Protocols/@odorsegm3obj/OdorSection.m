function [x, y] = OdorSection(obj, action, x, y)
   ...
   switch action
    case 'init',   % ---------- CASE INIT -------------
      ...
      NumeditParam(obj, 'tg_flow', 50, x, y, 'position', [x y 100 20], 'label', 'Tgt Flow', 'labelpos','left');
      NumeditParam(obj, 'bg_flow', 50, x, y, 'position', [x+100 y 100 20], 'label', 'Bgrd Flow', 'labelpos','left');
      set_callback({tg_flow,bg_flow}, {mfilename, 'write_olf'});
      next_row(y);
      
      MenuParam(obj, 'BG_bk',{1, 2, 3, 4}, 3, x, y, 'position', [x y 100 20], 'label', 'Bgrds Bank',...
          'labelpos', 'left', 'labelfraction', 0.6);      
      NumeditParam(obj, 'R_valve', 2, x, y, 'position', [x+100 y 100 20],...
          'label', 'R_Valve#', 'labelpos', 'left'); % set the vavle ID for the 'right' TG odors
      next_row(y);
      MenuParam(obj, 'TG_bk',{1, 2, 3, 4}, 3, x, y, 'position', [x y 100 20], 'label', 'Tgts Bank',...
          'labelpos', 'left', 'labelfraction', 0.6);
      NumeditParam(obj, 'L_valve', 1, x, y, 'position', [x+100 y 100 20],...
          'label', 'L_Valve#', 'labelpos', 'left'); % valve ID for the 'left' TG odors
      next_row(y);
      NumeditParam(obj, 'BG_valve', 1, x, y, 'label', 'Bkgrd_valve#','labelpos','left');
      next_row(y);
      ...
      SoloParamHandle(obj, 'valve_set', 'value', {(1:15), (1:15), (1:15), (1:15)});
      %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      NumeditParam(obj, 'bg8', 8, x, y, 'position', [x y 100 20],'label','BG 8','labelpos','left');
      MenuParam(obj, 'probe8', {'on', 'off'},2, x, y,'position', [x+100, y, 100, 20],'label','probe','labelpos','left');
      next_row(y); 
      ...
      OdorID_bg = [value(bg1); value(bg2); value(bg3); value(bg4); value(bg5); value(bg6); value(bg7);value(bg8)];
 ...
 %      for i = 2:length(OdorID_bg), bgids=[bgids ; i]; end;
      ...
      set_callback({bg1, bg2, bg3, bg4, bg5, bg6, bg7,bg8, probe1,probe2,probe3,probe4,...
          probe5,probe6,probe7,probe8}, {mfilename, 'write_olf'});
      MenuParam(obj, 'bgrd_ID', {1,2,3,4,5,6,7,8}, 1, x, y, 'position', [x y 100 20],...
      ...
      EditParam(obj, 'left_target', 'Ethyl Butyrate', x, y, 'position',[x y 100 40],...
          'label', 'Left Target',  'labelpos','top');
      EditParam(obj, 'right_target', '1-Propanol', x, y, 'position',[x+100 y 100 40],...
          'label', 'Right Target', 'labelpos','top');
      set_callback({ratio_variable, L_valve, R_valve, TG_bk, BG_bk}, {mfilename, 'update_odor'});
      ...
      switch hostname
          case 'cnmc7' % C1 (upper box)
              TG_bk.value = 2; BG_bk.value = 1;
              carrier_id.value = 3;
          case 'cnmc8' % C2 (lower box)
              TG_bk.value = 4; BG_bk.value = 3;
              carrier_id.value = 4;
      end
      % *******************************************************************
      % *******************************************************************
      ...
      SoloFunctionAddVars('make_and_upload_state_matrix', 'ro_args',...
          {'TG_bk','BG_bk','active_valve','BG_valve'})
   ...
   case 'write_olf'
       ...
       carrier_flow = 1000 - (value(tg_flow)+value(bg_flow));
       if ~strcmpi(value(OLF_IP), 'nan'), % if the olfactometer is connectable
              olf_meter.value = SimpleOlfClient(value(OLF_IP));
              Write(olf, ['Carrier' num2str(value(carrier_id)) '_Actuator'], carrier_flow);
              Write(olf, ['BankFlow' num2str(value(TG_bk)) '_Actuator'], value(tg_flow));
              Write(olf, ['BankFlow' num2str(value(BG_bk)) '_Actuator'], value(bg_flow));
       end
       OdorID_bg = [value(bg1); value(bg2); value(bg3); value(bg4); value(bg5); value(bg6); value(bg7);value(bg8)];
       OdorID_bg(find(OdorID_bg == 0)) = [];
       bgnames = odorset(OdorID_bg,2); 
   ...
   case 'update_odor'
     ...
     % Determine the odor valve ID to be opened.
      effective_bgvalve_set = valve_set{value(BG_bk)};
      BG_valve.value = effective_bgvalve_set(value(bgrd_ID));
