function [x, y] = PokesPlot(obj, action, x, y)
%
...
switch action,
 case 'init',  % ---------- CASE INIT ----------
   ...
   ToggleParam(obj, 'pokes_plot_show', 1, x, y, ...
     'position', [x y 100 20], ...
     'label', 'Pokes Plot show', 'TooltipString', ...
     'Show/hide PokesPlot window'); next_row(y);
   set_callback(pokes_plot_show, {mfilename,'view'});
   ...
   set(value(myfig), ...
       ...
       ['PokesPlot(' class(obj) '(''empty''), ''hide'')']);
   ...
   set_callback(export, {'PokesPlot', 'export'});
