function [out] = Probsamp10(varargin)
...
switch action
    ...
    case 'init'
        ...
        InitializeUIMenuParam('ExtraITIonError', {'0', '1', '2', '3', '4', '5', '6'},   1, rownum, colnum);   rownum = rownum+1;
        InitializeUIEditParam('TimeOutLength',                        0.25, rownum, colnum);   rownum = rownum+1;
        InitializeUIMenuParam('TimeOutSound',{'silence', 'white noise'}, 2, rownum, colnum);   rownum = rownum+1;
        rownum = rownum+0.5; % Blank row
        InitializeUIEditParam('MaxValidPokeDur',                       0.05, rownum, colnum);  rownum = rownum+1;
        InitializeUIEditParam('MinValidPokeDur',                       0.05, rownum, colnum);  rownum = rownum+1;
        ...
        InitializeUIEditHalfLeftParam( 'LeftSoundStart',                 2.5, rownum, colnum);
        InitializeUIEditHalfRightParam('LeftSoundEnd',                    10, rownum, colnum); rownum = rownum+1;
        SetParamUI(me, 'LeftSoundEnd', 'label', 'LeftSound');
        InitializeUIEditHalfLeftParam( 'RightSoundStart',                 10, rownum, colnum);
        InitializeUIEditHalfRightParam('RightSoundEnd',                  2.5, rownum, colnum); rownum = rownum+1;
        SetParamUI(me, 'RightSoundEnd', 'label', 'RightSound');
        InitializeUIEditParam('ValidSoundTime',                       0.03, rownum, colnum);   rownum = rownum+1;
        ...
        InitializeUIEditParam('LeftProb',                                1, rownum, colnum);   rownum = rownum+1;
        rownum = rownum+0.5; % Blank row
    ...
    case 'rewardports'
        if GetParam(me, 'RewardPorts') == 1,  
            ...
            SetParam(me, 'RightSoundStart', 10); SetParam(me, 'RightSoundEnd', 2.5);
            SetParam(me, 'LeftSoundStart', 2.5); SetParam(me, 'LeftSoundEnd',   10);
        end;
...
if side_list(ntrials+2)==1,
    StartFreq = GetParam(me, 'RightSoundStart')*1000;
    EndFreq   = GetParam(me, 'RightSoundEnd')*1000;
else
    StartFreq = GetParam(me, 'LeftSoundStart')*1000;
    EndFreq   = GetParam(me, 'LeftSoundEnd')*1000;
end;
