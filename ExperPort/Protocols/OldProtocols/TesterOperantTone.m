...
states_per_poke = 4;
ItiS1            = 0;                    % InterTrialInterval state
ItiS2            = 1;                    % InterTrialInterval state
prog_start       = 40;                    % state at which program starts
LrwS             = N*states_per_poke + prog_start;  % Left  Reward state
RrwS             = N*states_per_poke + prog_start;  % Right Reward state

SetParam(me, 'LrwS', LrwS); SetParam(me, 'RrwS', RrwS);

%          Cin    Cout    Lin    Lout    Rin    Rout    Tup    Timer    Dout    Aout
stm =     [ItiS2  ItiS2  ItiS2  ItiS2   ItiS2  ItiS2  prog_start iti      0       4] ; % InterTrial Interval State
stm=[stm;  ItiS1  ItiS1  ItiS1  ItiS1   ItiS1  ItiS1  prog_start iti      0       4] ; % InterTrial Interval State

stm = [stm ; zeros(prog_start-2,10)];

for i=0:N-1, 
    b  = i*states_per_poke + prog_start;      % base state for having poked i times
    nb = (i+1)*states_per_poke + prog_start;  % same for having poked i+1 times
