...
switch action
    ...
    case 'init'
        ...
        SetParamUI(me, 'LastCPokeMins', 'TooltipString', 'red = error noise; black = timeout noise; cyan = drink time; magenta = iti noise; green = appropriate time; blue = intertrial interval'); 
        ...
        InitParam(me, 'CenterPokeInState35_Flag', 'value', 0);
    ...
    case {'update', 'update_from_state_35'},
        ...
        for i=1:size(Event,1)
            ...
            if ismember(Event(i,2), [2]), % it was a center poke out
                ...
                if state==35, SetParam(me, 'CenterPokeInState35_Flag', 1); end;
...
function [] = initialize_centerpokes_plot
    ...
    b = line([0], [0]);  set(b, 'Color', 'b', 'Marker', '.', 'LineStyle', '-',    'Tag', 'bline');
    k = line([0], [0]);  set(k, 'Color', 'k', 'Marker', '.', 'LineStyle', 'none', 'Tag', 'kline');
    c = line([0], [0]);  set(c, 'Color', 'c', 'Marker', '.', 'LineStyle', 'none', 'Tag', 'cline');
    r = line([0], [0]);  set(r, 'Color', 'r', 'Marker', '.', 'LineStyle', 'none', 'Tag', 'rline');
    g = line([0], [0]);  set(g, 'Color', 'g', 'Marker', '.', 'LineStyle', 'none', 'Tag', 'gline');
    m = line([0], [0]);  set(m, 'Color', 'm', 'Marker', '.', 'LineStyle', 'none', 'Tag', 'mline');
...
function [] = update_centerpokes_plot
    ...
    error_state   = GetParam(me, 'itistart');
    nxitis        = length(GetParam(me, 'ExtraItiOnError', 'list'))-1;
    iti_state     = error_state + 2*nxitis;
    timeout_state = iti_state + 3;
    drinkl        = GetParam(me, 'LrwS') + 1;
    drinkr        = GetParam(me, 'RrwS') + 1;
    
    if ~isempty(h),
        vline = findobj(h, 'Tag', 'vpdline');
        bline = findobj(h, 'Tag', 'bline');
        rline = findobj(h, 'Tag', 'rline');
        gline = findobj(h, 'Tag', 'gline');
        kline = findobj(h, 'Tag', 'kline');
        cline = findobj(h, 'Tag', 'cline');
        mline = findobj(h, 'Tag', 'mline');
        
        if length(u)>0,
            set(bline, 'XData', CenterPokeTimes(u), 'YData', CenterPokeDurations(u));
            ...
            gu = find(CenterPokeStateHist(u) == GetParam(me, 'pstart'));
            set(gline, 'XData', CenterPokeTimes(u(gu)), 'YData', CenterPokeDurations(u(gu)));
            
            cu = find(CenterPokeStateHist(u)==drinkl  |  CenterPokeStateHist(u)==drinkr);
            set(cline, 'XData', CenterPokeTimes(u(cu)), 'YData', CenterPokeDurations(u(cu)));
            
            ru = find(error_state <= CenterPokeStateHist(u)  &  CenterPokeStateHist(u) < iti_state);
            set(rline, 'XData', CenterPokeTimes(u(ru)), 'YData', CenterPokeDurations(u(ru)));
            
            mu = find(iti_state <= CenterPokeStateHist(u)  &  CenterPokeStateHist(u) < timeout_state);
            set(mline, 'XData', CenterPokeTimes(u(mu)), 'YData', CenterPokeDurations(u(mu)));
            
            ku = find(timeout_state <= CenterPokeStateHist(u));
            set(kline, 'XData', CenterPokeTimes(u(ku)), 'YData', CenterPokeDurations(u(ku)));
...
if GetParam(me, 'CenterPokeInState35_Flag')==0,
    ...
    stm = [   0      0       0      0        0       0       1      itwt     0       0 ; ...
              0      0       0      0        0       0     pstart   iti      0    itne ; ...
        ];
end;
SetParam(me, 'CenterPokeInState35_Flag', 0);
...
if nxitis>1, 
    b = size(stm,1); b0 = b;
    stm = [stm ; ...
         b     b      b      b       b      b      1+b     itwt     0       0  ; ... 
         b     b      b      b       b      b      2+b     iti      0     itne];
end;
 
for i=2:nxitis,  % Extra Iti states: lower trig and play sound...
    b = size(stm,1);
    stm = [stm ; ...
         b0    b0     b0     b0      b0     b0     1+b     itwt     0       0  ; ... 
         b0    b0     b0     b0      b0     b0     2+b     iti      0     itne];
end;
        
stm = [stm ; ...
       ItiS   ItiS   ItiS   ItiS    ItiS   ItiS   1+ItiS   itwt     0       0 ; ... %16: lower trigs first
