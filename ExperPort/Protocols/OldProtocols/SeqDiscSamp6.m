function [out] = SeqDiscSamp6(varargin)
...
switch action
	...
	case 'init'
		...
		InitializeUIEditParam('SoundSPL',                       60, rownum, colnum);   rownum = rownum+1;
	...
	case {'presoundmeantime', 'presoundmintime', 'presoundmaxtime', ...
	      'sounddur', 'delay', 'delay2chord', 'chorddur'},
		...
		if GetParam('rpbox', 'state')==35, set_future_presoundtimes(GetParam(me, 'Trials')+1);
		else                               set_future_presoundtimes(GetParam(me, 'Trials')+2);
		end
	...
	case {'soundspl','basefreq','ntones','rampdur'},
		CheckParam( me, 'SoundSPL', 0, 70, 60);
...
SetRetriggerState( 'DeadTime', 'Tup', 'S35', 'Time', wntime, 'Aout', wnsound, 'Loop', 1 );
...
SetState( 'Start',        'Def', 'BaseState', 'Time', 0.01  );
SetState( 'BaseState',    'Cin', 'PreSound' );
SetState( 'PreSound',     'Def', 'BaseState',  'Tup', 'SampleSound', 'Time', prst );
SetState( 'SampleSound',  'Def', 'Withdrawal', 'Tup', 'PreChord', 'Time', sdur, 'Aout', 2 );

if (schedu==0) % no early moves allowed
	  SetState( 'PreChord', 'Def', 'Withdrawal', 'Tup', 'Chord', 'Time', wdur );	
...
if this_side==0, %left trial
	SetState( 'Chord',      'Tup', 'WaitForPoke', 'Time', cdur, 'Aout', 1, ...
		'Lin', 'Reward',    'Rin', 'NoReward' );
	SetState( 'WaitForPoke','Lin', 'Reward', 'Rin', 'NoReward' );
	SetState( 'Reward',     'Tup', 'Drink', 'Time', lwvt, 'Dout', 1 );
else
	SetState( 'Chord',      'Tup', 'WaitForPoke', 'Time', cdur, 'Aout', 1, ...
		'Lin', 'NoReward',  'Rin', 'Reward' );
...
SetState( 'Drink',               'Tup',  'ITIState',   'Time', drkt );
SetRetriggerState( 'ITIState',   'Tup',  'S35',      'Time', wntime, 'Aout', wnsound, 'Loop', itiloop );
SetRetriggerState( 'NoReward',   'Tup',  'ITIState', 'Time', wntime, 'Aout', wnsound, 'Loop', errorloop );
SetRetriggerState( 'Withdrawal', 'Tup',  'BaseState', 'Time', wntime, 'Aout', wnsound, 'Loop', withdrawalloop );
...
sounds = cell(3,1);
chord = MakeChord( 50e6/1024, 70-SPL, BaseFreq, NTones, ChordDur*1000, RampDur*1000 );
...
snd = Make2Sines( 50e6/1024, 70-SPL-6, f1*1000, f2*1000, SoundDur*1000, DelayDur*1000, RampDur*1000 );
