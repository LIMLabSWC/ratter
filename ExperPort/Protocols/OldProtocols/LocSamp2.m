function [out] = LocSamp2(varargin)
...
switch action
    ...
    case 'init'
		...
		InitializeUIEditParam('SoundDur',                              0.2, rownum, colnum);   rownum = rownum+1;
		InitializeUIEditParam('SoundSPL',                           60, rownum, colnum);   rownum = rownum+1;rownum = rownum+0.5; % Blank row
        ...
        snds = GetParam(me, 'Sounds');
        rpbox('LoadRP3StereoSound', snds);
	...
	case 'loadsettings',
		if load_uiparamvalues(me, GetParam(me, 'RatName')),
			...
			if GetParam('rpbox', 'state')==35, 
				...
				set_future_sides(NT+1);
                set_future_vpds(NT+1);
			end;
			update_plot;
		end;
    ...
    case 'state35',
        ...
        SetParam(me, 'Sounds', MakeSounds); rpbox('LoadRP3StereoSound', GetParam(me, 'Sounds')); 
