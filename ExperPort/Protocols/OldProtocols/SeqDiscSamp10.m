function [out] = SeqDiscSamp10(varargin)
...
% vClume (F1F2SPL) than the 'GO' signal 'GOSPL'.
% 091705 (SP): SeqDiscSamp9:
%   (1) New statistics (DispParams):
%       WdrawOverall - Overall % of trials that had 'Withdrawal'
%       WdrawOnCenter - % Centered 'GO' trials that had 'Withdrawal'
%   (2) FSM change: Previously, 'Withdrawal' would go back to
%   'BaseState' (re-initiate trial) instead of moving to another trial. The
%   current version has 'Withdrawal' --> 'ITIState' for tracking purposes.
%   (3) FSM change: The rats seem to continue poking during ITI. To
%   discourage this behaviour, ITI is broken into two states: ITIState and
%   ITIState2. Any poke behaviour causes each to go to the other and only
%   'Tup' in one or other goes to 'S35'.
% 091905: (3) from previous edit was changed back to have 'Withdrawal' -->
% 'ITIState'.
% 091905 (SP): SeqDiscSamp10: Incorporated Christian's poke-plot to view
% poke history for trials
%
...
switch action
    ...
    case 'init'
        ...
        load_uiparamvalues(me, 'cuauhtemoc');
        ...
        InitParam(me, 'Iti_List', 'value', zeros(1, maxtrials));
        set_future_trials(1);
        ...
        % parameters for schedule plot
        InitParam(me, 'LeftInPokeTimes', 'value', cell(1, maxtrials));
        InitParam(me, 'LeftOutPokeTimes', 'value', cell(1, maxtrials));
        InitParam(me, 'RightInPokeTimes', 'value', cell(1, maxtrials));
        InitParam(me, 'RightOutPokeTimes', 'value', cell(1, maxtrials));
        %      InitParam(me, 'RewardTimes', 'value', cell(1, maxtrials));
        InitParam(me, 'StartTimes', 'value', zeros(1,maxtrials));
        InitParam(me, 'RewardTrials', 'value', zeros(1,maxtrials));
        initialize_schedule_plot;
    ...
    case 'update',
        ...
        for i=1:size(Event,1)
            ...
            elseif Event(i,2)==3
                ...
                % for schedule_plot
                opt = GetParam( me, 'LeftOutPokeTimes' );
                onp = length( opt{NT} );
                ipt = GetParam( me, 'LeftInPokeTimes' );
                inp = length( ipt{NT} );
                ipt{NT}( inp + 1 ) = Event(i,3);
                if ( inp == onp+1 ) %last input didnt get its output, add one output
                    opt{NT}(inp) = ipt{NT}(inp);
                    SetParam( me, 'LeftOutPokeTimes', opt );
                end
                SetParam( me, 'LeftInPokeTimes', ipt );
            ...
            elseif Event(i,2)==4
                ...
                ipt = GetParam( me, 'LeftInPokeTimes' );
                inp = length( ipt{NT} );
                if (inp==onp)     %excess output, ignore.
                else
                    if (inp==onp+1)
                        opt{NT}( inp ) = Event(i,3);
                        SetParam( me, 'LeftOutPokeTimes', opt );
                        if opt{NT}(end)<ipt{NT}(end)
                            error( sprintf('this shouldnt happen: tinp = %d, tonp = %d', ipt{NT}(end), opt{NT}(end) ) );
                        end
                    else
                        error( sprintf('this shouldnt happen: inp = %d, onp = %d', inp, onp ) );
                    end
                end
            ...
            elseif Event(i,2)==5
                ...
                opt = GetParam( me, 'RightOutPokeTimes' );
                onp  = length( opt{NT} );
                ipt = GetParam( me, 'RightInPokeTimes' );
                inp = length( ipt{NT} );
                ipt{NT}( inp + 1 ) = Event(i,3);
                if ( inp == onp+1 ) %last input didnt get its output, add one output
                    opt{NT}(inp) = ipt{NT}(inp);
                    SetParam( me, 'RightOutPokeTimes', opt );
                end
                SetParam( me, 'RightInPokeTimes', ipt );
            ...
            end;    % end - Events for different poke types
            ...
            if isempty(current_hit),  % haven't figured out yet if this trial was a hit
                ...
                if Event(i,1)==WpkS | Event(i,1)==ChrdS,
                    if     ( (Event(i,2)==3 & current_side=='l') | (Event(i,2)==5 & current_side=='r') ),
                        ...
                        % for schedule_plot
                        rtrials = GetParam(me, 'RewardTrials');
                        rtrials(NT) = 1;    % reward
                        SetParam(me, 'RewardTrials', 'value', rtrials);
                ...
                elseif Event(i,1) == GetStateNumber('Withdrawal')
                    ...
                    SetParam(me, 'Withdraw_Trials', wdraw_list);

                    % for schedule_plot
                    rtrials = GetParm(me, 'RewardTrials');
                    rtrials(NT) = 2;    % abort: early withdrawal
                    SetParam(me, 'RewardTrials', 'value', rtrials);
                elseif Event(i,1) == GetStateNumber('BaseState') & Event(i,2) == 1  % trial starts on Cin
                    stt = GetParam(me, 'StartTimes');
                    stt(NT) = Event(i,3);
                    SetParam(me, 'StartTimes', stt);
                end;
            end; % end if empty(curr_hit)
...
% Store params for plot updates
iti_list = GetParam(me, 'Iti_List');
iti_list(starting_at:maxtrials) = GetParam(me,'ITI') * GetParam(me, 'WNTimeUnit');
SetParam(me, 'Iti_List', iti_list);
...
w_stat = 'n/a';
if total_surround > 0
    w_stat = length(ind1)/total_surround;
end;
SetParam(me, 'WdrawOnCenter', w_stat);
...
function plotbox_mini( xpos, y1, y2, col );
...
SetRetriggerState( 'ITIState',  'Def', 'ITIState2',  'Tup',  'S35',  'Time', wntime, 'Aout', wnsound, 'Loop', itiloop );    % trial up; play white noise; what's loop?
SetRetriggerState( 'ITIState2',  'Def', 'ITIState', 'Tup',  'S35',   'Time', wntime, 'Aout', wnsound, 'Loop', itiloop );   % ITI lasts as long as rat pokes in or out of any ports
...
%SetState('PauseState', 'Tup', 'ITIState', 'Time', wnsound+0.03);

% SP CHanges:
% InitState('ITIState2')
% ITIState should go to ITIState2 'Def', 'ITIState',
% Withdrawal should go to ITI, not BaseState

stm = CreateStateMatrix;
debug_fsm(stm, statematrix);
%error('here');
return;
...
function [] = initialize_schedule_plot

%Finally, the schedule
%h     = findobj(fig, 'Tag', 'plot_schedule');
%if ~isempty(h), delete(h); end

figure('Tag', 'schedule_fig', 'Position', [930 49 500 600]);
h = axes('Position', [0.1  0.1  0.85 0.85]);
...
function [] = update_schedule_plot

hitcolor = [0 1 0]; % - WORKS (bright green)
rewardcolor = hitcolor*0.5; % (pine-needle green)
misscolor = [1 0 0];    % (bright red)
wrongportcolor = misscolor * 0.5;   % (crimson)
earlycolor = [0.45 0.23 0]; % milk-chocolate brown
skipcolor = [0.8 0.8 0.8];  % (grey)
leftpokes_color = [1 0.4 0]; % (orange)
rightpokes_color = [1 1 0]; % WORKS (yellow)
%switchcolor = [0.8 0.8 0];
TrialsUsed = 50;    % SP: Someday, we might make this a UI param
XAxisLimit = 9;

fig   = findobj('Tag', 'schedule_fig');
h     = findobj(fig, 'Tag', 'plot_schedule');
if ~isempty(h),
    axes(h); cla;

    % Computing length of a trial
    stt      = GetParam(me, 'StartTimes' );
    presnd   = GetParam(me, 'PreSoundTimeList');
    sdur     = GetParam(me, 'SoundDur' );
    delay    = GetParam(me, 'Delay');
    d2c      = GetParam(me, 'Delay2ChordList');
    go_dur   = GetParam(me, 'ChordDur');

    % Get all event params
    iti_list = GetParam(me, 'Iti_List');
    NT       = GetParam(me, 'Trials');
    lipt     = GetParam(me, 'LeftInPokeTimes' );
    lopt     = GetParam(me, 'LeftOutPokeTimes' );
    ript     = GetParam(me, 'RightInPokeTimes' );
    ropt     = GetParam(me, 'RightOutPokeTimes' );

    %rwt      = GetParam(me, 'RewardTimes' );
    rtrials  = GetParam(me, 'RewardTrials' );
    Nused    = TrialsUsed; %GetParam(me, 'TrialsUsed' );
    strt     = max( NT-Nused, 1 );  % plot for this trial onwards

    % plot boxes of colour for different parts of a trial
    for k=strt:strt+Nused*1.5
        plotbox( k, 0, presnd(k), [0.5 0.5 0.5] ); % pre-sound poke

        % time points for different parts of trial
        beg = presnd(k) + (sdur*2) + delay + d2c(k) + go_dur; %rwt(k)+sdur(k);
        till_f2 = presnd(k)+(2*sdur)+delay;
        till_go = till_f2 + d2c(k);
        end_of_trial = till_go + go_dur + 3; % SP: Change - Reward availability

        plotbox( k, presnd(k), till_f2, [0.2 0.2 0.5] ); %f1-delay-f2 - WORKS (lilac)
        plotbox( k, till_f2,   till_go, [1 1 1] );       % delay till GO - WORKS (white)
        plotbox( k, till_go, till_go+go_dur, [0 1 0]);   % GO signal - WORKS (bright green)

        if (k==NT+1)
            plotbox( k, till_go+go_dur, end_of_trial, [1 0 0]); % Reward availability - WORKS
            plotbox( k, end_of_trial, end_of_trial+iti_list(k), [0.4 0.4 0.4] ); % - WORKS
            plotbox( k, end_of_trial+iti_list(k), XAxisLimit, [0.9 0.9 0.9] ); % - WORKS
        elseif (rtrials(k)==1) %reward
            plotbox( k, till_go+go_dur, end_of_trial, rewardcolor );     % 
            plotbox( k, end_of_trial, end_of_trial+iti_list(k), hitcolor );
        elseif (rtrials(k)==2) %abort: too early
            plotbox( k, till_go+go_dur, end_of_trial, earlycolor );
        elseif (rtrials(k)==3) %abort: wrong port
            plotbox( k, till_go+go_dur, end_of_trial, wrongportcolor );
            plotbox( k, end_of_trial, end_of_trial+iti_list(k), misscolor * 0.75 );
        else
            plotbox( k, till_go+go_dur, end_of_trial, [0.75 0.75 1] );  % slate-grey
            plotbox( k, end_of_trial, end_of_trial+iti_list(k), skipcolor );
        end
    end

    % now plot all the pokes
    for k=strt:NT
        for l=1:min(length(lopt{k}),length(lipt{k})),
            plotbox( k, lipt{k}(l)-stt(k), lopt{k}(l)-stt(k), leftpokes_color);
        end
        for l=1:min(length(ropt{k}),length(ript{k})),
            plotbox( k, ript{k}(l)-stt(k), ropt{k}(l)-stt(k), rightpokes_color );
        end
    end
    xlabel('Trial Time (sec)');
    ylabel('Trial No.');
    title('Task and Poke Timing');
    axis([-1 XAxisLimit strt-1 strt+Nused*1.5]);
    set(h, 'Tag', 'plot_schedule');
end
