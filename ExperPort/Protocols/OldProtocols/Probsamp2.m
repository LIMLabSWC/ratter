function [out] = Probsamp2(varargin)
...
switch action
    ...
    case 'init'
        ...
        FreqStart = [5000   8000];  %Hz
        FreqEnd   = [7000   6000];  %Hz
        Dur       = [250    250];  %msec
        ...
        n=n+.2;
        InitParam(me,'TotalReward','value',500);
        ...
        InitParam(me,'ValidTnPoke','ui','edit','value',200,'pos',[h+hs*1.8 n*vs hs*.6 vs]); n=n+1;
        ...
        InitParam(me,'LeftProb','ui','edit','value',0.5,'pos',[h+hs*1.8 n*vs hs*.6 vs]); n=n+1;
        ...
        SetParamUI(me,'TimeOutSound', 'label', 'TmOutSound');
        ...
        ValidTonePokeDur = ones(1,TotalReward)*GetParam(me,'ValidTnPoke');
        InitParam(me,'ValidTonePokeDur','value',ValidTonePokeDur);
        RewardAvailDur = ones(1,TotalReward)*GetParam(me,'RewardAvail');
        InitParam(me,'RewardAvailDur','value',RewardAvailDur);
        PortNumber = double(rand(1,TotalReward)<=GetParam(me,'LeftProb')); 
...
%         rpbox('send_matrix',state_transition_matrix(NextParam));
    ...
    case 'leftprob'
        ...
        new_portnumber = double(rand(1,GetParam(me,'TotalReward')-reward)<=GetParam(me,'LeftProb')); 
        lefts = find(new_portnumber); rights = find(~new_portnumber);
        PortNumber(lefts +reward) = GetParam(me, 'LeftSound');
        PortNumber(rights+reward) = GetParam(me, 'RightSound');
...
%  Cin Cout Lin   Lout Rin   Rout TimeUp        Timer DIO  AO  
    ...
    6    6   6      6   6      6    0           tmo   0      2;];% State 6 "TimeOut ==> House Light "
...
axis([0 40 0.5 2.5]);
xlabel('rewards'); ylabel('Port');
set(h, 'YTick', [1 2], 'YTickLabel', {'Lt' 'Rt'});
...
if ~isempty(h)
    ...
    PortNumber=GetParam(me,'PortNumber');
    plot(PortNumber,'b.'); hold on
    plot(reward+1,PortNumber(reward+1),'or');
...
function [snd] = make_timeout_sound()
    ...
    time = zeros(1, floor((50e6/1024)*timeoutdur));
