...
switch action,
 case 'init',  % ---------- CASE INIT ----------
  DispParam(obj, 'AvgPokeDur',    0, x, y, 'label', 'Total Poke Dur (avg)'); next_row(y); next_row(y,0.5);
  ...
  SoloParamHandle(obj, 'h2', 'saveable', 0, ...
                  'value', axes('Position', [0.08 0.55 0.145 0.1]));
 ...
 sound_len = ChordSection(obj, 'get_ChordSoundLen');
 go_dur = ChordSection(obj, 'get_GoDur');
 if n_done_trials == 0
 	poke_dur = (vpd(1) + sound_len);	% up to end of GO
else
 poke_dur = (vpd(n_done_trials) + sound_len);	% up to end of GO
 end;
 poke_dur = (poke_dur - go_dur) + vlst;
   
 AvgPokeDur.value = poke_dur;
