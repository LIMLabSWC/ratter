...
switch action,
    ...
    case 'init', % ----------  INIT  -------------------------
        ...
        SoloParamHandle(obj, 'h',  'value', axes('Position', [0.06, 0.82, 0.8, 0.12])); % axes
...
%         SoloParamHandle(obj, 'thm','value', text(-ones(1,maxtrials), 0.5*ones(1,maxtrials),'m'));
        SoloParamHandle(obj, 'thh','value', []);
        
        set_saveable({h;p;g;r;o;thh}, 0);
        set(value(h), 'DefaultTextHorizontalAlignment', 'Center', ...
          'DefaultTextVerticalAlignment', 'middle', 'DefaultTextFontSize', 8, ...
          'DefaultTextFontWeight', 'bold', 'DefaultTextColor', 'b', ...
          'DefaultTextFontName', 'Helvetica', 'DefaultTextClipping', 'on');
...
%         hold(value(h), 'on');
%         drawnow;
%         p.value = plot(value(h), n_done_trials+1:mx, side_list(n_done_trials+1:mx), 'b.'); 
        ...
        for i=1:u,
            ...
            if ismember(get_history(WaterDelivery, i), ...
                ...
                if hit_history(i)==1, thh(i) = text(i, side_list(i), 'h', 'Parent', value(h));
                else                  thh(i) = text(i, side_list(i), 'm', 'Parent', value(h));
                end;
