% All this protocol does is serve as a demo of the pokeplot plugin. You
% move from trial to trial by clicking twice in the center. On the second
% click, you enter "final_state", which lasts two secs, and is the only
% state in the "prepare_next_trial" set.
%
% This protocol should live in a file called
% Protcols/@PokesplotDemo/PokesplotDemo.m. 
%
% Make sure you ran newstartup, then dispatcher('init'), and you're good to
% go!
%

function [obj] = NXRigDemo(varargin)
...
switch action,
  ...
  case 'init'
    ...
    [x, y] = SoundInterface(obj, 'add', 'foo',x,y);
    ...
    sma = add_state(sma, 'name', 'wait_for_cpoke1', 'self_timer', 0.001, ...
...
%       'input_to_statechange', {'Cin', 'wait_for_cpoke2'});
  ...
  case 'prepare_next_trial'
    ...
    sma = StateMachineAssembler('full_trial_structure');
    sma = add_state(sma, 'name', 'wait_for_cpoke1', 'self_timer', .5, ...
        'output_actions',{'DOut',center1led},...
        'input_to_statechange', {'Tup', 'brain_stim'});
%     sma = add_state(sma, 'name', 'wait_for_cpoke1', ...
%       'input_to_statechange', {'Cin', 'wait_for_cpoke2'});
    sma = add_state(sma, 'name', 'brain_stim', 'self_timer', .1, ...
        'output_actions',{'DOut',stim2},...
        'input_to_statechange', {'Tup', 'final_state'});
    sma = add_state(sma, 'name', 'wait_for_cpoke2', ...
      'input_to_statechange', {'Cin', 'final_state'});
    sma = add_state(sma, 'name', 'final_state', ...
      'self_timer', 1.2, 'input_to_statechange', {'Tup', 'check_next_trial_ready'});
    dispatcher('send_assembler', sma, 'final_state');
