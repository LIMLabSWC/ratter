...
% -----------------------------------------------------------------------
  ...
  case 'prepare_next_trial',
    if n_done_trials >= 1,
        ...
        if size(parsed_events.states.left_reward,1) >= 1
            choice_history.value = [choice_history(:) ; 'l']; %#ok<NODEF>
        else
            choice_history.value = [choice_history(:) ; 'r']; %#ok<NODEF>
        end
        
        if     ~isfield(parsed_events.states,'after_lreward') && size(parsed_events.states.right_reward,1) == 0
            nonreward_history.value = [nonreward_history(:) ; 'l']; %#ok<NODEF>
        elseif ~isfield(parsed_events.states,'after_rreward') && size(parsed_events.states.left_reward,1)  == 0
            nonreward_history.value = [nonreward_history(:) ; 'r']; %#ok<NODEF>
        else
            nonreward_history.value = [nonreward_history(:) ; '.']; %#ok<NODEF>
        end
    else
        hit_history.value = [];
        choice_history.value = [];
    end;    
    %parsed_events.states
    %value(nonreward_history)
