...
try %#ok<TRYNC>
    ...
    n = bdata(['select n_done_trials from rigtrials where rigid=',num2str(rigid)]);
    if isempty(n)
        bdata(['insert into rigtrials set rigid=',num2str(rigid),', n_done_trials=0, peh=0, performance=0']);
    end
    ...
    if strcmp(do,'reset')
        bdata('call update_rigtrials("{Si}","{Si}","{S}","{S}")',rigid,0,0,[]);
    else
        bdata('call update_rigtrials("{Si}","{Si}","{S}","{S}")',rigid,ndt,perf,peh);
    end
end
